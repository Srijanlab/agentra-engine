...
- Dashboard GitHub-backed reads (`routes/apps.py`, `routes/review.py`) go through `gh_cache.cached(key, producer)`; any write that changes what they cover must call `gh_cache.invalidate_app(app)`. `/internal/rpc` does this automatically after any method in `_MEMORY_MUTATION_METHODS`, mapping `repo_url` -> app via its coordination repo.
- A parent feature issue is never marked code-complete while it has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` on both the `sub_feature_of` and `resolves_id` paths and returns `blocked_by_open_sub_issues` (>0 = not marked) (agentra#38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker comment; only comments after it can count as the answer, so an old consumed answer can't satisfy later re-escalations (agentra#54).
- `record_failure` dedups against any similar open bug (auth or not) and returns silently if one exists; otherwise auth failures and non-auth unfixable failures both escalate through `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), so the dashboard's "Needs your input" tab and Slack replies route (agentra#42/#46/#47).
...
- A new `Memory` method exposed over RPC that changes GitHub Issues/Projects state must also be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`, or the dashboard cache serves a stale view.
- A new `github_issues` function needs a counterpart on `FakeGitHubBackend` registered in `github_fake.install`, or tests using the fake hit the real GraphQL/REST path.
...
- `gh_cache.cached` serves the last known value (however stale) whenever the producer raises, so a GitHub outage shows as old data, not an error; only a key never cached before propagates the failure. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300], default 45s. Durable-layer `put`/`delete` swallow errors, so a bad `gh-cache` table silently degrades to per-instance caching.
- The dashboard cache is only busted by engine-mediated writes (`/internal/rpc` mutation methods, `apps.py` routes); a GitHub change made directly on github.com stays invisible until the TTL expires.
