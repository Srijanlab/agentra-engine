...
- `github_issues.get_issue`'s real REST payload has `labels` as `{"name": ..., ...}` objects, not plain strings — a bare `LABEL in issue["labels"]` silently always returns `False` against real GitHub (it only matches `github_fake`'s flat-string labels, which is why this went uncaught). Use `memory.core._label_names(issue)` to check label membership; `human_input_pending`'s previous bare-containment check made the escalation throttle guard re-post the same "Auto-escalated" comment every cycle instead of once (agentra#38/#15).
- `registry.loops.list_loops`/`_scan_loops`/`_query_loops_by_app` order by `_recency()` (`last_run_at`, stamped only by `roll_up_loop` on a real finished run) not `updated_at` — an administrative-only write (`set_loop_status`, `set_loop_pipeline`) also bumps `updated_at` and would otherwise shove every loop it touches to the top of the dashboard list regardless of real activity. The DynamoDB `by-app-recency` GSI still orders by `updated_at`, so the app-filtered query overfetches (`_RECENCY_OVERFETCH=200`) and re-sorts by `_recency()` in Python — don't trust the GSI's own order for anything user-facing.
