...
- `list_loops` 'most recently active' ordering is `last_run_at` (stamped only by `roll_up_loop`, i.e. a run actually completing), never `updated_at`: `set_loop_status`/`set_loop_pipeline` (used by retire/reconcile passes with no real run) also bump `updated_at`, so ordering on it would surface administratively-touched loops alongside genuinely active ones. Local sort, `_scan_loops`, and `_query_loops_by_app` (which overfetches past the `by-app-recency` GSI — that index only orders by `updated_at` — then re-sorts in Python) all key off the same `_recency()` helper.
...
- GitHub issue label lists from `github_issues.get_issue` are raw REST label *objects* (`{"name": ..., ...}`), not bare strings; extract names via `memory.core._label_names(issue)` rather than `label in issue["labels"]` directly.
...
- `github_fake`'s test-double labels are flat strings, but real GitHub REST labels are `{"name": ...}` objects; a naive `label in issue["labels"]` check passes against the fake but is always `False` against real GitHub. This exact bug made `human_input_pending` always return `False`, so the escalation-comment throttle never throttled (issues #38/#15 got 3-4 duplicate "Auto-escalated" comments) — always go through `_label_names()`.
