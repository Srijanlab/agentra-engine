...
- A parent issue with open sub-issues is never marked code-complete: `record_code_complete` checks `github_issues.open_sub_issue_count` (GraphQL `subIssuesSummary`) on both the sub-feature and `resolves_id` paths, and returns `blocked_by_open_sub_issues` (>0 means the parent was left unmarked) (issue #38).
...
- `record_failure` dedups every failure on `_find_similar_open_bug` (not just auth): a duplicate open bug returns without re-notifying. Auth failures and non-auth unfixable failures both escalate via `_escalate_blocking_failure` (thread-mapped Slack notify, human-input context, loop -> `waiting_for_human`) (#42/#46/#47).
- After a successful memory RPC whose method is in `_MEMORY_MUTATION_METHODS`, the engine calls `gh_cache.invalidate_app` for the coordination-repo's app so dashboard reads don't serve a pre-write view.
...
- Dashboard GitHub-backed read endpoints wrap their producer in `gh_cache.cached(key, producer)`. A new memory RPC method that changes GitHub state goes in `_MEMORY_MUTATION_METHODS` (`routes/internal.py`), and a new cache key family must be added to `gh_cache.invalidate_app`.
- A new `github_issues` function that `Memory` calls needs a `FakeGitHubBackend` counterpart wired in `github_fake.install`, or tests hit the real transport.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) it is served with only a log warning, so a GitHub outage shows arbitrarily old dashboard data. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` (default 45s, clamped 10–300), so edits made directly on GitHub lag by up to that long.
- Nothing in this repo creates the `gh-cache` DynamoDB table, and `_dynamo_store` get/put/delete swallow every exception, so a missing table silently degrades to the in-process layer only (cold instances always miss).
