...
- `list_loops` (both local-JSON and DynamoDB paths) orders by `loops._recency()` (`last_run_at` — stamped only by `roll_up_loop` when a run actually finishes — falling back to `created_at`), never bare `updated_at`; administrative-only writes like `set_loop_status`/`set_loop_pipeline` also bump `updated_at`, so sorting on it would shove every loop a retire/reconcile pass touches to the top of the dashboard's Loops list regardless of when it actually ran.
...
- The DynamoDB `by-app-recency` GSI is keyed on `updated_at`, not the real-activity `last_run_at` list_loops actually sorts by — `_query_loops_by_app` compensates by overfetching `_RECENCY_OVERFETCH=200` items off the index (Python-resorting by `_recency`) rather than trusting the GSI's own order; a burst of administrative touches across more than 200 loops for one app between real runs could still push the true top-N outside that window.
