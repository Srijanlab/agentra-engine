...
- Loop-list recency (`registry/loops.py::_recency`, used by `list_loops`/`_scan_loops`/`_query_loops_by_app`) sorts by `last_run_at` (stamped only by `roll_up_loop` on a real finished run), never `updated_at` — administrative-only writes (`set_loop_status`, `set_loop_pipeline`) bump `updated_at` but must not reorder the dashboard's Loops list.
...
- `github_issues.get_issue` returns the raw GitHub REST payload, whose `labels` are `{"name": ..., ...}` objects, not plain strings; `github_fake`'s labels are flat strings. A bare `LABEL in issue["labels"]` matches the fake but silently never matches real GitHub — always go through `memory.core._label_names(issue)`. (This exact mismatch made `human_input_pending` always return False in prod, re-posting the same escalation comment every cycle instead of once — agentra#38/#15.)
- The `loops` table's `by-app-recency` GSI is keyed/ordered by `updated_at`, not the real-activity-only `last_run_at` that `list_loops` actually sorts by; `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH = 200`) off the index and re-sorts by `_recency` in Python rather than trusting `ScanIndexForward` order directly.
