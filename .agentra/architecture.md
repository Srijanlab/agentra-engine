...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for dashboard GitHub reads: in-process dict over a durable store — `gh-cache` DynamoDB table in cloud mode, JSON under `AGENTRA_HOME/gh-cache/` otherwise); `human_gate/` (raises the `need_human` label + Slack post for runs whose `error`/`summary` carries `HUMAN_INPUT_REQUIRED`); `utils.py`.
...
- A terminal run (`completed|failed|blocked|waiting_for_human`) whose `error` or `summary` contains `HUMAN_INPUT_REQUIRED` must never go out silently (#55): `human_gate.maybe_raise(run_key)` runs after every successful `record_run` RPC and again via `sweep_recent_runs()` on each `/trigger/cron` tick. It is idempotent per `gate_run_key`, never raises, and counts an issue as announced only when a Slack thread is recorded — the `need_human` label alone is not proof.
- `record_code_complete` must not mark a parent (or a `resolves_id` issue) code-complete while `github_issues.open_sub_issue_count` > 0 (#38); it returns `blocked_by_open_sub_issues` instead.
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker; only comments after it can be an answer, so one consumed answer can't satisfy later escalations (agentra#54).
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; any RPC in `_MEMORY_MUTATION_METHODS` invalidates that app's keys via `gh_cache.invalidate_app`.
...
- A Memory method that mutates GitHub state and is exposed over RPC must also be added to `_MEMORY_MUTATION_METHODS` (`routes/internal.py`); a new `gh_cache` key covering an app must be added to `invalidate_app`'s list, and non-RPC write routes call `invalidate_app(app)` themselves.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) that value is served; only an empty cache propagates the error. Durable-store get/put/delete swallow all exceptions, so a missing `gh-cache` DynamoDB table silently degrades to in-process-only caching. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS`, clamped to [10, 300], default 45.
- RPC-driven cache invalidation only resolves an app when `repo_url` is a registered app's coordination repo (`_app_name_for_coordination_repo`); any other url is a silent no-op and the dashboard serves stale data until TTL.
- `record_failure` now dedups every failure (not just auth) against open bugs via `_find_similar_open_bug`: a recurring identical failure is still recorded but is not re-notified or re-escalated (#47).
