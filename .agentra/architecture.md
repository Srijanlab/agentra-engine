...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for dashboard GitHub reads: `__init__` exposes `cached`/`invalidate`/`invalidate_app`; `_inprocess` dict over a durable store — `_dynamo_store` (`gh-cache` table) in cloud mode, `_local_store` (JSON under `AGENTRA_HOME/gh-cache`) otherwise; `_ttl` reads `AGENTRA_GH_CACHE_TTL_SECONDS`); `utils.py`.
...
- A parent issue with open sub-issues is never marked code-complete: `record_code_complete` asks `github_issues.open_sub_issue_count` (GraphQL `subIssuesSummary`) before `mark_code_complete` on both the sub-feature and `resolves_id` paths, and reports the leftover as `blocked_by_open_sub_issues` in its return dict (issue #38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT `_HUMAN_INPUT_MARKER` comment only, so an already-consumed answer can't satisfy later re-escalations (agentra#54).
- A successful `memory` RPC whose method is in `_MEMORY_MUTATION_METHODS` invalidates that app's `gh_cache` entries (`invalidate_app`, resolved via the coordination repo URL), so dashboard reads don't serve a pre-write view.
...
- A new `Memory` method that changes GitHub issue/project state and is exposed over RPC goes in both `_MEMORY_METHODS` and `_MEMORY_MUTATION_METHODS`; a dashboard GitHub-read endpoint goes through `gh_cache.cached(key, producer)` and its key is added to `invalidate_app`.
- A new `connectors/github_issues.py` function that `Memory` calls needs a matching `FakeGitHubBackend` method registered in `github_fake.install`.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) it is served and the error swallowed, so a persistent GitHub failure shows as frozen dashboard data, not a 5xx. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300] (45s default); writes that bypass `_MEMORY_MUTATION_METHODS` / `invalidate_app` stay invisible until TTL. Durable-store get/put failures are swallowed too (best-effort), and the DynamoDB layer needs the `gh-cache` table to exist.
- `record_failure`'s `_find_similar_open_bug` dedup now gates every escalation (auth-failure and generic unfixable), not only auth: a recurring identical blocker records the bug but returns before notifying. `_escalate_blocking_failure` silently no-ops when the bug has no issue number, and it binds a loop and marks it `waiting_for_human` — so an unfixable failure now shows on the dashboard's "Needs your input" tab and is answerable via Slack/GitHub reply.
- `open_sub_issue_count` is a live GraphQL call inside `record_code_complete`'s catch-all: if GitHub errors, the method logs and returns `None` rather than marking code-complete.
