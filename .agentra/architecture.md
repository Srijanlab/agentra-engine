...
- "Most recently active" loop ordering is `registry.loops._recency()` (`last_run_at`, stamped only by `roll_up_loop` on a real finished run, falling back to `created_at`), never raw `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline`) also bumps `updated_at` but isn't activity.
...
- `github_issues.get_issue`'s real REST payload has `labels` as `{"name": ..., "color": ...}` objects, not plain strings — a bare `label in issue["labels"]` check silently and permanently returns `False` against real GitHub (only `github_fake`'s flat-string labels let that pass in tests). This is exactly what made `human_input_pending` never detect the `need_human` label in prod, so the escalation throttle guard re-posted the same "Auto-escalated" comment on every cycle instead of once (agentra#38/#15) until fixed to normalize via `_label_names(issue)`.
- `updated_at` on a loop is bumped by *any* write, including a purely administrative `set_loop_status`/`set_loop_pipeline` pass (e.g. a bulk retire/reconcile) that isn't a real run — sorting `list_loops` by `updated_at` shoves those stale loops to the top alongside genuinely active ones. `_recency()` fixes this by keying on `last_run_at`/`created_at` instead, but the DynamoDB `by-app-recency` GSI still only indexes `updated_at`, so `_query_loops_by_app` must overfetch (`_RECENCY_OVERFETCH=200`) off the index and re-sort in Python.
