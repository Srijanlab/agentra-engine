...
- `registry.loops` "most recently active" ordering is defined by `last_run_at` (stamped only by `roll_up_loop` on an actual completed run), never `updated_at` (also bumped by administrative-only writes like `set_loop_status`/`set_loop_pipeline`); `_query_loops_by_app` relies on `updated_at >= last_run_at` always holding so it can safely overfetch (`_RECENCY_OVERFETCH=200`) off the `by-app-recency` GSI (which is keyed on `updated_at`) and re-sort by `last_run_at` in Python.
...
- `github_issues.get_issue` returns GitHub's real REST label shape (`{"name": ..., ...}` objects), not the flat strings `github_fake` uses — a bare `LABEL in issue["labels"]` check (as `human_input_pending` used to do) silently always returns `False` against real GitHub, only ever matching against `github_fake`'s shape. Always compare via `memory.core._label_names(issue)`. This bug slipped through because tests only exercised `github_fake`, and in prod it made the `need_human` escalation-comment throttle a no-op, reposting the same "Auto-escalated" comment on every single cycle (agentra#38, #15).
