...
- `github_issues.get_issue` returns the raw GitHub REST payload, whose `labels` are `{"name": ..., ...}` objects, not plain strings — a bare `LABEL in issue["labels"]` only matches `github_fake`'s flat-string labels and silently returns `False` against real GitHub. Normalize through `memory.core._label_names(issue)` first; this bit `human_input_pending`, which re-posted the same "Auto-escalated" comment every cycle instead of once (agentra#38/#15).
- `registry.loops.list_loops` sorts by `last_run_at` (stamped only by `roll_up_loop`, i.e. a real completed run), not `updated_at` — an administrative-only write like `set_loop_status`/`set_loop_pipeline` (e.g. a bulk retire/reconcile pass) also bumps `updated_at` and would otherwise shove days-old, already-resolved loops to the top of the dashboard's Loops list alongside genuinely fresh ones. The DynamoDB `by-app-recency` GSI still orders by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH`=200) and re-sorts in Python.
