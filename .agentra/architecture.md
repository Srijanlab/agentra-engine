...
- Any GitHub Issues label-membership check must go through `memory.core._label_names(issue)`, never `LABEL in issue["labels"]` directly — see Gotchas.
...
- `github_issues.get_issue`'s raw REST payload has `labels` as `{"name": ..., "color": ..., ...}` objects, not plain strings (only `github_fake` uses flat strings). A bare `LABEL in issue["labels"]` silently always returns `False` against real GitHub and only ever passes in tests — this shipped undetected in `human_input_pending()` and made the `need_human` escalation-comment throttle a no-op, re-posting "Auto-escalated" on every cycle instead of once (agentra#38, #15). Always compare against `core._label_names(issue)`.
- `registry.loops.list_loops` sorts by `last_run_at` (stamped only by `roll_up_loop`, i.e. a run actually finished), deliberately not `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a bulk retire/reconcile pass) also bumps `updated_at` and would otherwise shove every loop it touches, days-old and fresh alike, to the top of the dashboard together. The DynamoDB `by-app-recency` GSI still only orders by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH=200`) off the index and re-sorts by `last_run_at` in Python — if a single administrative pass ever touches more than ~200 loops for one app in between real runs, the re-sort window can miss the true top-N.
