...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for the dashboard's GitHub-backed reads: in-process dict over the DynamoDB `gh-cache` table or local JSON, stale-on-error); `human_gate/` (raises the human gate for terminal runs whose `error`/`summary` carries `HUMAN_INPUT_REQUIRED`); `utils.py`.
...
- A terminal run (`completed|failed|blocked|waiting_for_human`) whose `error` or `summary` contains `HUMAN_INPUT_REQUIRED` must never go out silently (#55): `human_gate.maybe_raise` runs after every successful `record_run` RPC and `sweep_recent_runs` re-checks recent runs on each `/trigger/cron` tick. It is idempotent per `gate_run_key` and never raises.
- `record_code_complete` never marks a parent/`resolves_id` issue code-complete while `open_sub_issue_count(...) > 0`; it returns `blocked_by_open_sub_issues` instead (#38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT `_HUMAN_INPUT_MARKER` comment, so an old answered escalation can't satisfy later ones (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for both auth and generic unfixable failures; only a not-yet-reported one escalates, via `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`) (#42/#46/#47).
...
- A whitelisted `_MEMORY_METHODS` entry that mutates GitHub Issues/Projects state must also go in `_MEMORY_MUTATION_METHODS` (`routes/internal.py`) so the app's `gh_cache` entries are invalidated after the RPC.
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; writes that change what they cover call `gh_cache.invalidate` / `invalidate_app`. Do the route's own 404/validation before calling `cached()`.
...
- `gh_cache.cached` serves the last known value (however stale) when its producer raises, and DynamoDB `gh-cache` put/get/delete errors are swallowed — a GitHub or table outage shows up as stale dashboard data, not a 5xx; only a key with no prior value propagates the error. `invalidate_app` also busts the shared `digest_batch` entry.
- `human_gate` reads the token from run `error` first, then free-text `summary`: a crashed tool call can end a run as `completed` with no `error` at all (#49/#50/#51). A `need_human` label alone doesn't prove an issue was announced; only a recorded Slack thread does (`slack_thread_for`).
