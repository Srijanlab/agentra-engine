...
- `Memory.human_input_pending()` must match GitHub's real label shape: `github_issues.get_issue` returns the raw REST payload where `labels` are `{"name": ..., ...}` objects, not plain strings. A bare `_NEED_HUMAN_LABEL in issue["labels"]` never matches real GitHub (it only matched `github_fake`'s flat-string labels, which is why this went uncaught by tests) and silently always returned `False` — the escalation-comment throttle then re-posted the same "Auto-escalated" comment on every single cycle instead of once (confirmed live on issues #38 and #15). Fixed by matching through `_label_names(issue)`; any new label-membership check on a real GitHub issue payload must use the same helper, not a raw `in`.
- `registry.loops.list_loops` orders by `last_run_at` (stamped only by `roll_up_loop`, i.e. a real run actually finishing), not `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a bulk retire/reconcile pass) also bumps `updated_at` and would otherwise shove every loop it touches, days-old and freshly-resolved alike, to the top of the dashboard's Loops list together (confirmed live). The DynamoDB `by-app-recency` GSI is still ordered by `updated_at` only, so `_query_loops_by_app` compensates by overfetching `_RECENCY_OVERFETCH` (200) rows and re-sorting by real recency in Python — a `Limit` smaller than the true top-N by `last_run_at` would silently drop entries.
