...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for the dashboard's GitHub-backed reads: `__init__.py` exposes `cached`/`invalidate`/`invalidate_app`; `_inprocess` dict over a durable `_dynamo_store` (`gh-cache` table, cloud) or `_local_store` (JSON under `AGENTRA_HOME/gh-cache/`); `_ttl`, `_entry`); `utils.py`.
...
- A parent feature issue is never marked code-complete while `github_issues.open_sub_issue_count` > 0: `record_code_complete` checks this on both the sub-feature and `resolves_id` paths and reports the count as `blocked_by_open_sub_issues` in its return dict (issue #38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT `_HUMAN_INPUT_MARKER` comment only; an answer to an earlier escalation must never satisfy a later one (agentra#54).
- `Memory.record_failure` dedups on `_find_similar_open_bug` for every failure class: a similar open bug means no new notify/escalation. A first-seen auth failure or unfixable failure goes through `_escalate_blocking_failure` (thread-mapped Slack notify, human-input context, loop `waiting_for_human`), so it shows in the dashboard's "Needs your input" tab (issues #42/#46/#47).
- Every successful `memory` RPC in `_MEMORY_MUTATION_METHODS` invalidates that app's `gh_cache` entries (`invalidate_app`, resolved via the coordination repo URL).
...
- Dashboard read endpoints (`apps.py` detail/digest, `review.py` board/ready-to-review) go through `gh_cache.cached(key, producer)`; keys are `app_detail:{app}`, `backlog_board:{app}`, `ready_to_review:{app}`, `digest_batch`. A new write path that changes what they show calls `invalidate_app`, and a new memory-mutating RPC method must be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`.
- A new function on `connectors/github_issues.py` used by memory needs a `FakeGitHubBackend` counterpart in `github_fake.py`, patched in `install()`, or tests hit the real GraphQL path.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) it is served instead, so a GitHub outage looks like frozen dashboard data, not a 5xx. Only a key with no prior value propagates the error. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to 10–300s (default 45s).
- The durable `gh_cache` layer swallows every DynamoDB error (`get` returns `None`, `put`/`delete` pass) and expects a `<prefix>gh-cache` table to exist; a missing table silently degrades to per-instance in-process caching only, so cold Vercel instances re-hit GitHub.
- A memory-mutating RPC method missing from `_MEMORY_MUTATION_METHODS` doesn't error; the dashboard just serves the pre-write view until the TTL expires. The shared `digest_batch` key is busted wholesale by any `invalidate_app`.
- `/trigger/deploy-complete` reuses the tick bearer check (`AGENTRA_INTERNAL_TOKEN` / `CRON_SECRET`), and like `/trigger/cron` it is open if neither env var is set. It enqueues a cycle with `enforce_schedule=False`, and `repo`/`sha` in the payload are logged, never trusted.
