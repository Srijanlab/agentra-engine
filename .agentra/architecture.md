...
- `github_issues.get_issue()` returns the raw GitHub REST payload, whose `labels` are `{"name": ..., ...}` objects, not plain strings — `github_fake`'s labels are flat strings, so a bare `LABEL in issue["labels"]` check passes against the fake but always misses against real GitHub. `human_input_pending()` had exactly this bug (always returned `False` in prod), so the escalation-throttle guard re-posted the same "Auto-escalated" comment every single cycle instead of once (confirmed live on issues #38, #15) until fixed to go through `memory.core._label_names(issue)`.
- `registry.list_loops` (and its DynamoDB `_scan_loops`/`_query_loops_by_app` backends) sort by `last_run_at` — stamped only by `roll_up_loop`, i.e. a real finished run — not by `updated_at`. An administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a bulk retire/reconcile pass) also bumps `updated_at`, which previously shoved every touched loop, days-old and fresh alike, to the top of the dashboard's Loops list together. The `by-app-recency` GSI still only orders by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH=200`) off the index and re-sorts by `last_run_at` in Python — don't shrink that overfetch without checking it still exceeds any realistic single-app admin-touch burst.
