...
- A parent feature issue is never marked code-complete while it still has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` on both the sub-feature and `resolves_id` paths and returns `blocked_by_open_sub_issues` (>0 = not marked) alongside `issue_number`/`board_issue_number` (GitHub #38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker comment, never the first, so an old already-consumed answer can't satisfy every later escalation (agentra#54).
- A successful `/internal/rpc` memory call whose method is in `_MEMORY_MUTATION_METHODS` invalidates that app's `gh_cache` entries (`invalidate_app`), resolved via its coordination repo URL.
...
- A new `Memory` method that changes GitHub Issues/Projects state and is exposed over RPC must also be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`, or the dashboard serves a stale cached view. Engine-side writes (e.g. `routes/apps.py`) call `gh_cache.invalidate_app(name)` themselves.
- Dashboard GitHub-backed read endpoints go through `gh_cache.cached(key, producer)`; a one-arg producer receives the previous etag and may return `FetchResult` for conditional refresh. Per-app keys are `app_detail:`/`backlog_board:`/`ready_to_review:` + app, plus the shared `digest_batch`; new per-app keys must be added to `invalidate_app`.
...
- Blocking-failure escalations (`record_failure`: auth failure, generic unfixable failure) go through `MemoryIssuesMixin._escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), not a bare Slack post.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) it is served and the error only logged; only a key with no prior value propagates it. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300], default 45s. Durable-layer `put`/`delete` failures in `_dynamo_store` are swallowed, so a missing `gh-cache` table silently degrades to in-process-only caching.
- `record_failure`'s dedup now uses `_find_similar_open_bug` for every failure class: an identical open bug suppresses re-notification/re-escalation (auth and generic unfixable alike), and escalation is skipped when `record_known_bug` returns no issue number (`_escalate_blocking_failure` returns early on `issue_number is None`).
