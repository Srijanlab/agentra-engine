...
- `registry.loops.roll_up_loop` stamps `last_run_at` only when a real run actually finished; `list_loops`/`_scan_loops`/`_query_loops_by_app` order by `last_run_at` (falling back to `created_at`), never by `updated_at`, so an administrative-only write (`set_loop_status`, `set_loop_pipeline`) can't reorder the loops list.
...
- Any code checking whether a real GitHub issue carries a label must go through `memory.core._label_names(issue)` (which normalizes the REST `{name: ...}` label objects to strings) rather than testing membership against `issue["labels"]` directly.
...
- `github_issues.get_issue()` returns GitHub's raw REST label shape (`labels: [{"name": ..., "color": ..., ...}]`), not flat strings — a bare `LABEL in issue["labels"]` silently always returns `False` against real GitHub (it only matches `github_fake`'s flat-string labels, which is why this shipped unnoticed). `human_input_pending` had exactly this bug: the escalation-comment throttle never actually detected the `need_human` label, so the same "Auto-escalated" comment got re-posted every cycle instead of once (agentra#38, #15) until it was fixed to go through `_label_names()`.
- The DynamoDB `by-app-recency` GSI is keyed by `updated_at`, but `list_loops` needs `last_run_at` ordering — `_query_loops_by_app` over-fetches (`_RECENCY_OVERFETCH=200` rows off the index) and re-sorts by `_recency()` in Python rather than trusting `ScanIndexForward` order directly; a `limit` larger than the overfetch window would silently under-fetch.
