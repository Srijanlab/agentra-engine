...
- Loop-list recency is real-activity-only: `registry/loops.py::_recency()` sorts by `last_run_at` (stamped exclusively by `roll_up_loop` when a run actually completes) falling back to `created_at`, never `updated_at` — an administrative-only write (`set_loop_status`, `set_loop_pipeline`) must not reorder the dashboard's Loops list.
...
- `github_issues.get_issue` returns the raw GitHub REST payload, whose `labels` are `{"name": ...}` objects, not plain strings — a bare `X in issue["labels"]` silently always returns `False` against real GitHub (it only matches `github_fake`'s flat-string labels, which is why this went uncaught). Use `memory.core._label_names()`. This exact bug made `human_input_pending`'s escalation-comment throttle a no-op in prod (repeated "Auto-escalated" comments on agentra#38/#15) until fixed.
...
- The DynamoDB `by-app-recency` GSI still orders by `updated_at`, not the real-activity `last_run_at` that `list_loops` sorts by; `_query_loops_by_app` compensates by overfetching `_RECENCY_OVERFETCH` (200) rows off the index and re-sorting by `_recency` in Python — correct only as long as no more than ~200 loops for one app get administratively touched at once.
