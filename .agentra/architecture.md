...
- `agentra/memory/` — per-repo product state. `Memory` = 5 mixins (issues, issue_lifecycle, features, settings, specs); GitHub Issues/Projects-backed. `core.py` holds label constants (incl. the `status:awaiting-testing` label and its legacy `status:shipped` alias) + `_label_names()` (GitHub REST label-object -> name-string helper) + failure-triage regexes (transient / unfixable-by-agentra / login-required) + spec-header helpers + `pipeline_stages()` (the dashboard's single ordered list of pipeline columns).
...
- `registry.loops.list_loops`'s "most recently active" ordering is driven by `last_run_at` (stamped only by `roll_up_loop`, i.e. a run actually finished), never `updated_at` — administrative-only writes (`set_loop_status`, `set_loop_pipeline`) must not reorder the dashboard's Loops list.
...
- GitHub issue label-membership checks go through `memory.core._label_names(issue)`, not a bare `label in issue["labels"]` — see Gotchas.
...
- `github_issues.get_issue`'s `labels` are real GitHub REST objects (`{"name": ..., ...}`), not plain strings. `human_input_pending` used to do a bare `_NEED_HUMAN_LABEL in issue["labels"]`, which always returned False against real GitHub (only `github_fake`'s flat-string labels matched, which is why it went uncaught) — silently breaking the escalation-comment throttle and reposting the same "Auto-escalated" comment every cycle. Fixed to use `memory.core._label_names(issue)`; any new label-membership check must do the same.
- `registry.loops.list_loops` sorts by `_recency()` = `last_run_at` (set only by `roll_up_loop` on a finished run) falling back to `created_at`, deliberately not `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a retire/reconcile pass) also bumps `updated_at` and previously shoved stale, already-resolved loops to the top of the dashboard's Loops list alongside genuinely fresh ones. The DynamoDB `by-app-recency` GSI still orders by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH=200`, `updated_at` is always >= `last_run_at`) and re-sorts by `_recency` in Python — don't trust the GSI's own order for loops.
