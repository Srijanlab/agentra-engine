...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for the dashboard's GitHub-backed reads in `apps.py`/`review.py`: `_inprocess` dict over a durable store — `_dynamo_store` (`gh-cache` table) in cloud mode, `_local_store` JSON under `AGENTRA_HOME/gh-cache/` otherwise; `_ttl.py`, `_entry.py`); `utils.py`.
...
- `record_code_complete` never marks a parent/tracking issue code-complete while `open_sub_issue_count` > 0; it returns `blocked_by_open_sub_issues` instead. Applies to both the `sub_feature_of` and `resolves_id` paths.
- `find_unanswered_human_input_comment` anchors to the MOST RECENT `_HUMAN_INPUT_MARKER` comment and only considers later non-internal comments, so an already-consumed answer can't satisfy a later re-escalation (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for both escalation paths: an identical open bug means no re-notify/re-escalate. Non-auth unfixable failures escalate via `_escalate_blocking_failure` (thread-mapped Slack notify, human-input context, loop `waiting_for_human`), same as auth failures (#42/#46/#47).
...
- A new `Memory` method that mutates GitHub Issues/Projects state and is exposed over RPC must also go in `routes/internal.py::_MEMORY_MUTATION_METHODS`, so `gh_cache.invalidate_app` runs after it; routes that write directly call `invalidate_app` themselves.
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; a producer may take one arg (previous etag) to do conditional fetches and return `FetchResult`.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) it is served, so a GitHub outage shows old data rather than a 5xx; only a never-cached key propagates the error. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300], default 45s. Durable-store reads/writes swallow all exceptions, so a missing `gh-cache` table silently degrades to in-process-only caching per Lambda instance.
- `/trigger/deploy-complete` is in `_PUBLIC_PREFIXES` (bypasses the Firebase gate) and relies solely on `_verify_tick_auth`; it enqueues a cycle with `enforce_schedule=False` and treats the caller's `repo`/`sha` as log-only.
