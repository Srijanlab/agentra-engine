...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for dashboard GitHub reads: `__init__.py` holds `cached`/`invalidate`/`invalidate_app`, `_inprocess`/`_dynamo_store`/`_local_store` are the layers, `_ttl`, `_entry`); `human_gate/` (`maybe_raise` / `sweep_recent_runs` — raises the need_human label + Slack post for runs that end with a `HUMAN_INPUT_REQUIRED` note); `utils.py`.
...
- A run that ends terminal with `HUMAN_INPUT_REQUIRED` in its `error` or `summary` must never go out silently (agentra#55): `human_gate.maybe_raise(run_key)` runs after every successful `record_run` RPC, and `sweep_recent_runs` re-checks recent runs on each `/trigger/cron` tick. It is idempotent per `gate_run_key` and never raises.
- A parent issue with open sub-issues is never marked code-complete: `record_code_complete` checks `github_issues.open_sub_issue_count` (GraphQL `subIssuesSummary`) and returns `blocked_by_open_sub_issues` instead (agentra#38).
- `record_failure` dedups on `_find_similar_open_bug` before any notification, so a recurring identical blocker escalates once. Both the auth-failure and the generic unfixable path escalate through `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop marked `waiting_for_human`) (agentra#42/#46/#47).
...
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; a `Memory` method reachable over RPC that mutates GitHub Issues/Projects state must be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py` so the app's cache entries are invalidated (`gh_cache.invalidate_app`). A new cache key covering an app's data must be added to `invalidate_app`.
- Any new `github_issues` function a `Memory` path calls needs a matching `FakeGitHubBackend` method and an entry in `github_fake.install`'s patch list.
...
- `gh_cache.cached` serves the last known value however stale when its producer raises (only a never-cached key propagates the error), so a GitHub outage shows old dashboard data instead of a 5xx. `_invalidate_gh_cache_for_rpc` resolves the app via its coordination `repo_url`; a URL matching no registered app invalidates nothing.
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker, not the first. Anchoring to the first let one already-consumed answer satisfy every later escalation, producing an unattended escalate -> false-resume -> re-throttle bounce (agentra#54).
- `human_gate._raise` treats `need_human` plus a recorded Slack thread as the proof that an ask was announced. A label alone (e.g. from a throttle escalation that never touched loop state) still gets a Slack post and `waiting_for_human` loop state, and a stale `slack_thread_ts` from an already-answered gate must not be reused for a new one.
