...
- `agentra/memory/` — per-repo product state. `Memory` = 5 mixins (issues, issue_lifecycle, features, settings, specs); GitHub Issues/Projects-backed. `core.py` holds label constants (incl. the `status:awaiting-testing` label and its legacy `status:shipped` alias, and `_label_names()` for normalizing a real GitHub issue's label objects) + failure-triage regexes (transient / unfixable-by-agentra / login-required) + spec-header helpers + `pipeline_stages()` (the dashboard's single ordered list of pipeline columns).
...
- `list_loops` orders "most recently active" by `last_run_at` — stamped only by `roll_up_loop`, i.e. a run actually finished — never by `updated_at`. Administrative-only writes (`set_loop_status`/`set_loop_pipeline`, e.g. a bulk retire/reconcile pass) bump `updated_at` on every loop they touch but must not reorder the dashboard's Loops list; `_recency()` is the single sort key everywhere (local, `_scan_loops`, `_query_loops_by_app`).
...
- Any label check against a `github_issues.get_issue`/`get_pull` result goes through `memory.core._label_names(issue)`, never a bare `label in issue["labels"]` — real GitHub returns label objects, only `github_fake` returns flat strings.
...
- `github_issues.get_issue` returns the raw GitHub REST payload, whose `labels` are `{"name": ...}` objects, not plain strings, so a bare `label in issue["labels"]` silently always returns `False` against real GitHub (it only worked against `github_fake`'s flat-string labels). This left `human_input_pending()` always returning `False`, so the escalation-comment throttle guard re-posted the same "Auto-escalated" comment on every cycle instead of once (confirmed live on agentra#38 and #15) until it was routed through `_label_names()`.
- The `by-app-recency` DynamoDB GSI is keyed on `updated_at`, not the real-activity-only `last_run_at`; `_query_loops_by_app` deliberately overfetches (`_RECENCY_OVERFETCH` = 200 rows) off the index and re-sorts by `_recency()` in Python rather than trusting GSI order for the final page.
