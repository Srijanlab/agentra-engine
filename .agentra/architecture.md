...
- `roll_up_loop` stamps `last_run_at` only when a real run actually finishes; `updated_at` is bumped by that too but also by administrative-only writes (`set_loop_status`, `set_loop_pipeline`). Every loop-listing path (`list_loops`, `_scan_loops`, `_query_loops_by_app`, via the shared `_recency()` key) sorts by `last_run_at`, never `updated_at`, so a bulk status/pipeline touch can't shove stale loops to the top of the dashboard's Loops list.
...
- Any check of an issue's labels goes through `memory.core._label_names()`, never a bare `in` against the raw `labels` field — GitHub REST returns label objects, not strings.
...
- `github_issues.get_issue` returns raw GitHub REST label objects (`{"name": ...}`), while `github_fake` uses flat strings — a bare `label in issue["labels"]` check passes against the fake but silently always returns `False` against real GitHub. `issue_lifecycle.human_input_pending` did exactly this and re-posted the same escalation comment every cycle instead of once (agentra#38/#15) until fixed to go through `_label_names()`.
- The `by-app-recency` DynamoDB GSI itself still orders by `updated_at`, not `last_run_at`; `_query_loops_by_app` compensates by overfetching `_RECENCY_OVERFETCH` (200) items off the index and re-sorting by `_recency()` in Python — correct only as long as fewer than ~200 loops for a single app get administratively touched between real runs.
