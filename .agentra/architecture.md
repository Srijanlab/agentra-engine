...
- `agentra/memory/` — per-repo product state. `Memory` = 5 mixins (issues, issue_lifecycle, features, settings, specs); GitHub Issues/Projects-backed. `core.py` holds label constants (incl. the `status:awaiting-testing` label and its legacy `status:shipped` alias) + a `_label_names()` normalizer for GitHub label payloads + failure-triage regexes (transient / unfixable-by-agentra / login-required) + spec-header helpers + `pipeline_stages()` (the dashboard's single ordered list of pipeline columns).
...
- Label membership checks against a `github_issues.get_issue()` payload must go through `memory.core._label_names(issue)`, never a bare `LABEL in issue["labels"]` — real GitHub's `labels` are `{"name": ..., ...}` objects, not strings (only `github_fake`'s labels are flat strings). `issue_status` and `human_input_pending` follow this.
...
- Until recently `human_input_pending`'s bare `_NEED_HUMAN_LABEL in issue["labels"]` check always evaluated False against real GitHub (only matched `github_fake`'s flat-string labels), so the escalation-comment throttle in `_reconcile_human_input_for_app` never saw the label as present and re-posted the same 'Auto-escalated' comment every single cron tick instead of once (issues #38, #15). Now fixed via `_label_names()`, but the same footgun awaits any new code that indexes `get_issue()`'s `labels` list directly.
- `list_loops` orders by `_recency()` (`last_run_at`, stamped only by `roll_up_loop` on a real finished run), not `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a bulk retire/reconcile pass) bumps `updated_at` but must not reorder the dashboard's loop list. The DynamoDB `by-app-recency` GSI is still keyed on `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH`=200 items) off the index and re-sorts by `_recency` in Python rather than trusting GSI order directly.
