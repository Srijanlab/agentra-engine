...
- Dashboard GitHub-backed reads (`app_detail:{app}`, `backlog_board:{app}`, `ready_to_review:{app}`, shared `digest_batch`) go through `gh_cache.cached`; any write that changes them must call `invalidate_app(app)`. Memory RPCs listed in `_MEMORY_MUTATION_METHODS` do this automatically after success, resolving the app from the coordination `repo_url`.
- A parent feature is never marked code-complete while it still has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` on both the sub-feature and `resolves_id` paths, and returns `blocked_by_open_sub_issues` (>0 when it held back) alongside `issue_number`/`board_issue_number` (GitHub #38).
- `record_failure` dedups on any similar open bug (`_find_similar_open_bug`), not just auth failures: a recurring identical blocker neither re-notifies nor re-escalates. Non-auth unfixable failures and auth failures both escalate via `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`) so replies route and the dashboard's "Needs your input" tab shows them (#42/#46/#47).
...
- A new `_MEMORY_METHODS` entry that mutates GitHub Issues/Projects state must also go in `_MEMORY_MUTATION_METHODS` (same file), or the dashboard cache keeps serving the pre-write view until TTL.
...
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker and only considers comments after it. Anchoring to the first marker let one old answer satisfy every later re-escalation, driving an unattended escalate -> false-resume -> re-throttle bounce (agentra#54).
...
- `gh_cache.cached` serves the last known value (however stale) whenever the producer raises, and only a key with no prior value propagates the error. Dashboard views can lag GitHub by `AGENTRA_GH_CACHE_TTL_SECONDS` (default 45s, clamped 10–300), and much longer during a GitHub outage. Only engine-side writes invalidate: edits made directly on GitHub stay stale until the TTL expires. Both durable stores swallow write errors, so a broken `gh-cache` table degrades silently to in-process-only caching.
- `record_failure`'s `already_reported` early-return now applies to every failure, so an identical open bug suppresses re-escalation even if the earlier escalation was never answered. `_escalate_blocking_failure` is a no-op when `issue_number` is None.
