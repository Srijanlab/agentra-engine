...
- `list_loops` ordering depends on `last_run_at` being stamped only by `roll_up_loop` (i.e. an actual completed run) — administrative-only writes (`set_loop_status`, `set_loop_pipeline`) bump `updated_at` but must never touch `last_run_at`, or a bulk retire/reconcile pass would again shove untouched-in-ages loops to the top of the dashboard's Loops list.
...
- "Most recently active" loop/run ordering lives in `registry.loops._recency()` (last real run, not last write); don't sort loop lists by `updated_at` directly.
- GitHub issue label membership checks go through `memory.core._label_names(issue)`, never a bare `LABEL in issue["labels"]` — see Gotchas for why.
...
- `github_issues.get_issue` returns the raw GitHub REST payload — `labels` are `{"name": ..., ...}` objects, not plain strings, while `github_fake` uses flat strings. A bare `LABEL in issue["labels"]` only ever matches the fake and silently returns `False` against real GitHub: `human_input_pending`'s `need_human` check had exactly this bug, so the escalation throttle re-posted the same "Auto-escalated" comment every cycle instead of once (confirmed live on agentra#38/#15) until it was fixed to go through `_label_names(issue)`.
- The DynamoDB `by-app-recency` GSI itself still orders by `updated_at`, not `last_run_at`; `_query_loops_by_app` compensates by overfetching (`_RECENCY_OVERFETCH = 200`) and re-sorting in Python — correct only as long as fewer than ~200 loops for one app get administratively touched between real runs.
