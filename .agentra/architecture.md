...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (two-layer read-through TTL cache for the dashboard's GitHub-backed reads: in-process dict over a durable store — DynamoDB in cloud mode, local JSON otherwise; serves the last value if the producer raises); `human_gate/` (`maybe_raise` / `sweep_recent_runs` — raises the `need_human` label + Slack post when a terminal run's `error`/`summary` carries `HUMAN_INPUT_REQUIRED`); `utils.py`.
...
- `agentra/connectors/` — GitHub App (`github_app.py` mints a per-`owner/repo` installation token), issues/pulls/projects/variables/issue-lifecycle mutations (incl. `migrate_awaiting_testing_label`, the one-time label-rename migration, and `open_sub_issue_count`), `github_fake.py`, Slack (`slack.py` + `slack_allowlist.py`).
...
- A parent feature issue is never marked code-complete while it still has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` on both the `sub_feature_of` and `resolves_id` paths and reports the count as `blocked_by_open_sub_issues` in its return dict (#38).
- A run asking for a human must not end silently: a terminal run whose `error` or `summary` contains `HUMAN_INPUT_REQUIRED` gets its gate raised idempotently by `human_gate.maybe_raise` — right after a successful `record_run` RPC, and again by `sweep_recent_runs` on each `/trigger/cron` tick as the backstop (#55).
- A `need_human` label alone is not proof a gate was announced; `human_gate` treats a recorded Slack thread (`registry.slack_thread_for`) as the only proof, and starts `loop.human_input` clean for a fresh gate so a stale `slack_thread_ts` can't suppress the new post.
...
- A `Memory` method exposed over RPC that changes GitHub Issues/Projects state must also be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`, or the dashboard's `gh_cache` entries for that app stay stale. Route-side writes call `gh_cache.invalidate_app(app)` themselves.
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; a producer may take one arg (the previous etag) to send a conditional request.
...
- A blocking failure that needs a human is escalated through `MemoryIssuesMixin._escalate_blocking_failure` (thread-mapped Slack notify + `record_human_input_context` + loop marked `waiting_for_human`), not an ad-hoc Slack post — both the auth-failure and generic unfixable-failure paths in `record_failure` use it.
- A new `Memory`/`github_issues` function needs a matching `github_fake.FakeGitHubBackend` method and an entry in `github_fake.install`'s patch list, or tests hit the real transport.
...
- `gh_cache.cached` swallows producer errors whenever any prior value exists (even long-expired) and returns it, so a persistent GitHub failure shows stale dashboard data with no 5xx; only a never-cached key propagates the error.
- `find_unanswered_human_input_comment` anchors on the MOST RECENT `_HUMAN_INPUT_MARKER` comment; only comments after it count as answers. Anchoring on the first marker let one consumed answer satisfy every later escalation (agentra#54, escalate -> false-resume bounce).
- `record_failure` now dedups on `_find_similar_open_bug` for both the auth and generic unfixable paths: an identical still-open bug records but does NOT re-notify/re-escalate (#47). Fixing the bug's wording/diagnosis changes the match and re-escalates.
- `human_gate` reads the token from `run['error']` first, then `run['summary']`: a crashed tool call can end a run as `completed` with no `error`, so the ask lives only in the free-text summary (#49/#50/#51).
- `open_sub_issue_count` is a GraphQL call (`subIssuesSummary`) made on every non-`more_parts_expected` `record_code_complete`; it raises rather than returning 0 on GraphQL errors, which `record_code_complete`'s outer try turns into a logged `None` return.
