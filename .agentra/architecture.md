...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache package for the dashboard's GitHub-backed reads: `__init__` exposes `cached`/`invalidate`/`invalidate_app`, over `_inprocess` dict + a durable store — `_dynamo_store` (`gh-cache` table) or `_local_store` (`AGENTRA_HOME/gh-cache/*.json`); `_ttl`, `_entry`); `utils.py`.
...
- Dashboard GitHub-backed reads (`/apps` digest, app detail, backlog board, ready-to-review) go through `gh_cache.cached(key, producer)`: in-process dict over a durable store, fresh for `AGENTRA_GH_CACHE_TTL_SECONDS` (clamped 10–300s, default 45). If the producer raises, the last cached value is served (stale-on-error); only a key with no prior value propagates the error. Keys are `app_detail:{app}`, `backlog_board:{app}`, `ready_to_review:{app}`, `digest_batch`.
- Any write that changes GitHub issue/project state must bust that cache: `/internal/rpc` memory calls in `_MEMORY_MUTATION_METHODS` call `invalidate_app` (resolved via the coordination repo's app) after success, and app routes call `_invalidate_app_cache`. Invalidation is best-effort and never fails the write.
- A parent feature issue is never marked code-complete while it has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` and returns `blocked_by_open_sub_issues` (>0 means it was skipped) (issue #38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker and only considers comments after it, so an old already-consumed answer can't satisfy a later re-escalation (agentra#54).
- `record_failure` escalates an unfixable non-auth failure the same way as an auth failure (`_escalate_blocking_failure`: thread-mapped Slack notify, human-input context, loop marked `waiting_for_human`), and both are skipped when `_find_similar_open_bug` already finds an open match (issue #47).
...
- A new `Memory` method that mutates GitHub state and is exposed to the loop goes in both `_MEMORY_METHODS` and `_MEMORY_MUTATION_METHODS`; a new cached dashboard read key must be added to `gh_cache.invalidate_app`.
- A new function on `connectors/github_issues.py` that memory code calls needs a matching `FakeGitHubBackend` method and a line in `github_fake.install`'s patch list.
...
- `gh_cache` hides GitHub outages behind stale data (no 5xx, no signal beyond a warning log), and both durable stores swallow every read/write error. A GitHub-state write that bypasses `_MEMORY_MUTATION_METHODS` / `_invalidate_app_cache` (or whose `repo_url` isn't a registered coordination repo) leaves the dashboard stale for up to the TTL.
- `/trigger/deploy-complete` and `/trigger/cron` are `_PUBLIC_PREFIXES` paths guarded only by `_verify_tick_auth`, which is open when neither `AGENTRA_INTERNAL_TOKEN` nor `CRON_SECRET` is set. `deploy-complete` enqueues a cycle with `enforce_schedule=False`; `repo`/`sha` in its payload are logged, never trusted.
- Because `record_failure`'s `already_reported` check now gates every escalation, a recurring identical blocker is recorded on the existing bug but not re-notified on Slack after the first time.
