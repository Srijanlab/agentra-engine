...
- A run that ends terminal (`completed|failed|blocked|waiting_for_human`) with `HUMAN_INPUT_REQUIRED` in its `error` or `summary` must not go out silently: `human_gate.maybe_raise(run_key)` sets `need_human`, records human-input context and posts a thread-mapped Slack message. It runs from the `/internal/rpc` `record_run` handler (fast path) and `human_gate.sweep_recent_runs()` on each `/trigger/cron` tick (backstop); it is idempotent per run via `human_input.gate_run_key` and never raises (agentra#55).
- `record_code_complete` never marks a parent / `resolves_id` issue code-complete while `github_issues.open_sub_issue_count` > 0; it returns `blocked_by_open_sub_issues` instead (issue #38).
- Any `_MEMORY_MUTATION_METHODS` RPC that succeeds invalidates that app's `gh_cache` entries (`invalidate_app`), so dashboard reads don't serve a stale board.
...
- `registry` sub-modules re-export through `registry/__init__.py`'s `__all__`; when adding one, update the `routes/internal.py` whitelist in the same change if the loop needs it. A new GitHub-mutating `Memory` method exposed over RPC must also be added to `_MEMORY_MUTATION_METHODS`.
- Routes are `APIRouter`s included at root prefix in `server/__init__.py`; auth exemptions go in `auth.py::_PUBLIC_PREFIXES`.
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; a route write that changes what a view covers calls `gh_cache.invalidate_app(app)`. Route-level errors (e.g. unregistered-app 404) are raised before `cached()`.
...
- Human-blocking failures escalate through `MemoryIssuesMixin._escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`); don't post a bare Slack message.
...
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker; only comments after it count as an answer. Anchoring to the first marker let one consumed answer satisfy every later escalation (agentra#54).
- `record_failure` now dedups on `_find_similar_open_bug` for both auth and generic unfixable failures: a recurring identical blocker is recorded but not re-escalated (issue #47).
- `gh_cache.cached` serves the last known value (however stale) when the producer raises, so a GitHub outage shows old data rather than a 5xx; only a key never cached propagates the error. `invalidate` clears the durable layer and the calling instance's in-process dict only — other warm instances keep their in-process entry until TTL.
