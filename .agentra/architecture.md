...
- Any code checking a GitHub issue's labels off `github_issues.get_issue(...)` must go through `memory.core._label_names(issue)`, not a bare membership test against `issue["labels"]` — the real API returns label objects, not strings.
- "Recently active" for loops means `last_run_at` (stamped only by `roll_up_loop` on a real completed run), not `updated_at` (also bumped by administrative-only writes like `set_loop_status`/`set_loop_pipeline`); any new loop-listing/sorting code must key off `_recency`, matching `registry/loops.py`.
...
- `github_issues.get_issue` returns the raw GitHub REST payload, whose `labels` are `{"name": ...}` objects, not plain strings. `memory.issue_lifecycle.human_input_pending` used to test `_NEED_HUMAN_LABEL in issue["labels"]` directly — that only matches `github_fake`'s flat-string labels, so against real GitHub it always returned `False` and the escalation throttle re-posted the same "Auto-escalated" comment every single cycle instead of once (confirmed live on issues #38 and #15). Fixed via `memory.core._label_names(issue)`; any new label-membership check must use it too.
- DynamoDB's `by-app-recency` GSI on the loops table is ordered by `updated_at`, not `last_run_at` — `_query_loops_by_app` over-fetches `max(limit, 200)` rows off the index and re-sorts by `_recency` in Python rather than trusting `ScanIndexForward` order directly; raising `limit` well past 200 without also raising the overfetch window risks a stale loop sorting ahead of a fresh one if enough loops get administratively touched at once.
