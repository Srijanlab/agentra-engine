...
- `record_code_complete` never marks a parent `code_complete` while `github_issues.open_sub_issue_count` is >0 — on both the `sub_feature_of` path and a `resolves_id` naming a parent — and reports it as `blocked_by_open_sub_issues` in its return value (issue #38).
- `find_unanswered_human_input_comment` only considers comments after the MOST RECENT `_HUMAN_INPUT_MARKER`; anchoring to the first marker let one consumed answer satisfy every later re-escalation (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for both auth-failure and generic unfixable failures: a match still records the bug but suppresses re-notify. A new one goes through `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), the same treatment as `_escalate_to_human` (issues #42/#46/#47).
- Dashboard GitHub-backed reads (`app_detail:<app>`, `backlog_board:<app>`, `ready_to_review:<app>`, shared `digest_batch`) go through `gh_cache.cached`; a producer error serves the last cached value (stale-on-error), and only a key with no prior value lets it propagate.
...
- `/trigger/deploy-complete` (a code repo's CI calls it as its last deploy step) is bearer-gated like `/trigger/cron`, and reuses `_enqueue_cycle(..., enforce_schedule=False)` rather than a new enqueue path.
...
- A `_MEMORY_METHODS` entry that mutates GitHub Issues/Projects state must also be added to `_MEMORY_MUTATION_METHODS` (`routes/internal.py`) so `rpc` evicts that app's `gh_cache` entries; a dashboard route write outside RPC calls `invalidate_app` itself (`apps.py::_invalidate_app_cache`). A new cached read needs its key added to `invalidate_app`.
- A new `github_issues` function that `Memory` calls needs a `FakeGitHubBackend` counterpart wired into `github_fake.install()`, or tests hit the real transport.
...
- Dashboard reads can be up to `AGENTRA_GH_CACHE_TTL_SECONDS` stale (default 45s, clamped to [10, 300]), and indefinitely stale while GitHub is failing. RPC eviction only fires after a successful mutation call and only when `repo_url` matches a registered coordination repo. The durable `gh_cache` stores swallow every read/write/delete error, so a broken `gh-cache` table degrades silently to the in-process layer.
