...
- Label membership checks always go through `memory.core._label_names(issue)` (or the derived `_at_awaiting_testing_stage`), never a bare `label in issue["labels"]` — real GitHub REST payloads carry `{"name": ...}` label objects, only `github_fake`'s are flat strings.
- Loop-list recency is sorted by `registry/loops.py::_recency()` (real activity — `last_run_at`, stamped only by `roll_up_loop` on an actual run) rather than `updated_at`, which administrative-only writes (`set_loop_status`, `set_loop_pipeline`) also bump; follow the same distinction for any new "most recently active" ordering.
...
- `github_issues.get_issue`'s labels are `{"name": ...}` objects, not strings; a bare `X in issue["labels"]` matches only `github_fake`'s flat-string labels and silently always evaluates False against real GitHub. This previously let `human_input_pending()` miss the `need_human` label and re-post the same escalation comment on every cycle instead of once (live on issues #38/#15) until fixed to route through `_label_names()`.
- The `by-app-recency` DynamoDB GSI is keyed on `updated_at`, not the real-activity `last_run_at` that `list_loops` now sorts by; `_query_loops_by_app` compensates by over-fetching (`_RECENCY_OVERFETCH=200`) off the index and re-sorting in Python — an app with >200 loops touched administratively between real runs could still mis-order.
