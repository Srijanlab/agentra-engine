...
- `agentra/memory/` — per-repo product state. `Memory` = 5 mixins (issues, issue_lifecycle, features, settings, specs); GitHub Issues/Projects-backed. `core.py` holds label constants (incl. the `status:awaiting-testing` label and its legacy `status:shipped` alias) + failure-triage regexes (transient / unfixable-by-agentra / login-required) + spec-header helpers + `pipeline_stages()` (the dashboard's single ordered list of pipeline columns) + `_label_names()` (normalizes GitHub's real `{name: ...}` label objects vs. plain strings).
...
- `github_issues.get_issue`'s `labels` are raw GitHub REST `{"name": ...}` objects, not plain strings — matching must go through `memory.core._label_names()`; a bare `LABEL in issue["labels"]` always misses against real GitHub (it only ever worked against `github_fake`'s flat-string labels), which let `human_input_pending` re-post the same escalation comment on every cycle instead of once before this was caught (agentra#38/#15).
- `registry.loops.list_loops` sorts by `_recency()` (`last_run_at` if the loop has ever run, else `created_at`), never bare `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline` from a retire/reconcile pass, no real run) also bumps `updated_at` and previously shoved every loop it touched to the top of the dashboard's Loops list together. The DynamoDB `by-app-recency` GSI still only orders by `updated_at`, so `_query_loops_by_app` overfetches `_RECENCY_OVERFETCH=200` rows off the index and re-sorts by `_recency()` in Python before truncating to `limit`.
