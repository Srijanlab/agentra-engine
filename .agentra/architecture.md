...
- `agentra/connectors/` — GitHub App (`github_app.py` mints a per-`owner/repo` installation token), issues/pulls/projects/variables/issue-lifecycle mutations (incl. `open_sub_issue_count` via GraphQL `subIssuesSummary`, and `migrate_awaiting_testing_label`, the one-time label-rename migration), `github_fake.py` (test double patched over `github_issues` in `install()`), Slack (`slack.py` + `slack_allowlist.py`).
...
- `record_code_complete` never marks a parent feature code-complete while `github_issues.open_sub_issue_count` > 0 (both the sub-issue path and the `resolves_id` path); it returns `blocked_by_open_sub_issues` so the caller can see why (issue #38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker comment, so an old consumed answer can't satisfy later re-escalations (agentra#54).
- A successful memory RPC in `_MEMORY_MUTATION_METHODS` invalidates that app's `gh_cache` entries (`invalidate_app`), so dashboard reads don't serve a view older than the write.
...
- `registry` sub-modules re-export through `registry/__init__.py`'s `__all__`; when adding one, update the `routes/internal.py` whitelist in the same change if the loop needs it. A whitelisted memory method that mutates GitHub state must also go in `_MEMORY_MUTATION_METHODS`.
- A new GitHub-backed function in `connectors/github_issues.py` needs a `github_fake` counterpart wired in `install()`.
- Dashboard GitHub-backed read endpoints go through `gh_cache.cached(key, producer)`; a write that changes what a key covers calls `invalidate` / `invalidate_app`. A one-arg producer receives the previous etag and may return `FetchResult` for conditional refresh.
...
- Blocking-failure escalations from `record_failure` (auth failure, unfixable failure) go through `MemoryIssuesMixin._escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`); don't call `slack.notify_human_input_required` directly for them.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists, the old value is served with only a log warning, so a GitHub outage shows the dashboard old data rather than a 5xx. Only a key with no prior value propagates the error. Durable-layer reads/writes/deletes swallow exceptions, so a broken `gh-cache` table degrades silently to in-process-only caching.
- Cache freshness otherwise relies on TTL (`AGENTRA_GH_CACHE_TTL_SECONDS`, clamped to 10-300s, default 45s). RPC invalidation only fires for methods in `_MEMORY_MUTATION_METHODS` and only when `repo_url` maps to a registered app's coordination repo; a mutating method missing from that set serves stale reads until TTL. `invalidate_app` also drops the shared `digest_batch` key.
- `record_failure`'s `_find_similar_open_bug` dedup now gates every escalation, not just auth failures: a recurring identical blocker records the bug but does not re-notify or re-escalate, so a lost first Slack message is never resent.
