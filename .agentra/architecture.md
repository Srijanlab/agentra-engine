...
- `agentra/memory/` — per-repo product state. `Memory` = 5 mixins (issues, issue_lifecycle, features, settings, specs); GitHub Issues/Projects-backed. `core.py` holds label constants (incl. the `status:awaiting-testing` label and its legacy `status:shipped` alias) + failure-triage regexes (transient / unfixable-by-agentra / login-required) + spec-header helpers + `pipeline_stages()` (the dashboard's single ordered list of pipeline columns) + `_label_names()` (normalizes real GitHub's `{name:...}` label objects and `github_fake`'s flat strings to a set of names).
...
- `registry.loops.list_loops` orders by `last_run_at` (stamped only by `roll_up_loop`, i.e. an actual run finishing), not `updated_at` (also bumped by administrative-only writes like `set_loop_status`/`set_loop_pipeline`) — an administrative retire/reconcile pass touching many loops at once must not reorder the dashboard's recency list.
...
- Checking whether a GitHub issue carries a label goes through `memory.core._label_names(issue)`, never a bare `LABEL in issue["labels"]` — real GitHub returns `{name: ...}` label objects while `github_fake` returns flat strings, and the bare form silently only works against the fake.
...
- GitHub REST label payloads are `{"name": ..., ...}` objects, not plain strings; `github_fake`'s labels are flat strings, so code (and tests written against the fake) can look correct while a bare `LABEL in issue["labels"]` silently never matches real GitHub. `human_input_pending` had exactly this bug — it always returned `False` against real GitHub, so the escalation-comment throttle re-posted the same "Auto-escalated" comment on every cycle instead of once (agentra#38/#15). Use `_label_names(issue)`.
- The DynamoDB `by-app-recency` GSI is still keyed on `updated_at`, not the true-activity `last_run_at`; `_query_loops_by_app` overfetches (`Limit=max(limit, 200)`) off the index and re-sorts in Python by `_recency`. If more than 200 loops for one app get administratively touched between real runs, the true top-N by `last_run_at` could fall outside that overfetch window.
