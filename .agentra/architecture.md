...
- `list_loops` ranks by `_recency` = `last_run_at` (stamped only by `roll_up_loop` when a run actually finishes) falling back to `created_at`, never `updated_at` — administrative-only writes (`set_loop_status`, `set_loop_pipeline`) still bump `updated_at` but must not reorder the dashboard's Loops list; a bulk retire/reconcile pass touching many loops at once previously did exactly that.
...
- The DynamoDB `by-app-recency` GSI is still keyed on `updated_at`, not the real-activity `last_run_at` — `_query_loops_by_app` compensates by overfetching `_RECENCY_OVERFETCH` (200) items off the index and re-sorting by `_recency` in Python. This is only correct as long as fewer than 200 of one app's loops get administratively touched between real runs; a bigger bulk-retire pass could push a genuinely-recent loop out of the fetched window.
