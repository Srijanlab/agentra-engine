...
- Loop recency ordering is driven by `last_run_at`, stamped only by `roll_up_loop` when a run actually finishes — never by `updated_at`, which any write (including administrative-only ones like `set_loop_status`/`set_loop_pipeline`) also bumps.
...
- Any comparison against a GitHub issue's labels must go through `memory.core._label_names(issue)`, never a bare `label in issue["labels"]` — real GitHub returns label objects (`{"name": ...}`), only `github_fake` returns flat strings.
- `registry/loops.py` keeps two separate timestamps per loop: `updated_at` (any write) and `last_run_at` (stamped only by `roll_up_loop`, i.e. a real run finishing). Anything that orders loops by recency (`list_loops`, `_scan_loops`, `_query_loops_by_app`) must sort by the shared `_recency()` helper (`last_run_at` else `created_at`), not `updated_at`.
...
- Before the `_label_names()` fix, `issue_lifecycle.human_input_pending`'s bare `_NEED_HUMAN_LABEL in issue["labels"]` check silently always returned `False` against real GitHub (only matched `github_fake`'s flat-string labels), so the escalation-comment throttle never engaged and the same "Auto-escalated" comment got re-posted every cycle (confirmed live on agentra#38/#15). Any new label-membership check must go through `_label_names()`, not a bare `in`.
- The `by-app-recency` DynamoDB GSI is still keyed on `updated_at`, not `last_run_at`. `_query_loops_by_app` compensates by over-fetching `max(limit, 200)` rows off the index and re-sorting by `_recency()` in Python — correct as long as fewer than ~200 loops for one app get an administrative-only touch (bumping `updated_at` without a real run) between the true top-`limit` by `last_run_at` and the end of that window; a bigger simultaneous administrative sweep could push a genuinely-recent loop out of the fetched page.
