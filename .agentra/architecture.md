...
- `list_loops` orders by `last_run_at` (stamped only inside `roll_up_loop`, i.e. a run actually finished), never by `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline` from a retire/reconcile pass) also bumps `updated_at` and must not reorder the dashboard's Loops list. The DynamoDB `by-app-recency` GSI is still keyed by `updated_at`, so `_query_loops_by_app` overfetches (`max(limit, 200)`) off the index and re-sorts by `last_run_at` in Python.
...
- `github_issues.get_issue`'s `labels` are raw GitHub REST objects (`{"name": ..., "color": ..., ...}`), not plain strings — a bare `LABEL in issue["labels"]` silently always returns `False` against real GitHub (it only matches `github_fake`'s flat-string labels, which is why it went uncaught). Label-membership checks must go through `memory.core._label_names(issue)`. This exact bug in `human_input_pending` made the escalation throttle guard fail open, reposting the same "Auto-escalated" comment every cycle (confirmed live on agentra#38 and #15).
