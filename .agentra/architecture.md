...
- A run that reaches a terminal status with `HUMAN_INPUT_REQUIRED` in `error` (preferred) or `summary` must never go out silently (agentra#55): `human_gate.maybe_raise(run_key)` is called after every successful `record_run` RPC and swept over recent runs on each `/trigger/cron` tick. It is idempotent (keyed on the loop's `human_input.gate_run_key` + `slack_thread_ts`), never raises, and skips an issue that already has an announced (Slack-threaded) open gate.
- A multi-part feature's parent issue is never marked code-complete while `github_issues.open_sub_issue_count` > 0 (agentra#38); `record_code_complete` returns `blocked_by_open_sub_issues` so the caller can tell.
- Non-auth unfixable failures from `record_failure` are escalated like every other human-input block (`_escalate_blocking_failure`: thread-mapped Slack, human-input context, loop -> `waiting_for_human`); a similar already-open bug suppresses re-escalation (agentra#47).
...
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; a route that changes what they cover calls `gh_cache.invalidate_app(app)`, and a new whitelisted memory RPC method that mutates GitHub issue/project state must be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py` so the RPC path invalidates too.
...
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker; anchoring to the first let one already-consumed answer satisfy every later escalation, driving an unattended escalate -> false-resume -> re-throttle bounce (agentra#54).
...
- `gh_cache` is stale-on-error: if a producer raises, the last cached value (however old) is served and the error only propagates on a key with no prior entry. The DynamoDB layer swallows every read/write error (a missing `gh-cache` table just looks like a permanent miss), and invalidation is best-effort — a failed bust leaves a stale view until TTL.
