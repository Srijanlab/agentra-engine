...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for the dashboard's GitHub-backed reads: `__init__.py` `cached`/`invalidate`/`invalidate_app`, in-process layer + durable layer `_dynamo_store` (`gh-cache` table) / `_local_store`, `_ttl.py`, `_entry.py` `FetchResult`); `human_gate/` (`maybe_raise` / `sweep_recent_runs`: raises the human gate for a terminal run whose error/summary carries `HUMAN_INPUT_REQUIRED`); `utils.py`.
...
- A run must never end asking a human silently (#55): `human_gate.maybe_raise(run_key)` runs after every successful `registry.record_run` RPC and again from `sweep_recent_runs` on each `/trigger/cron` tick; it is idempotent per `gate_run_key` (fully done once the loop's `human_input.slack_thread_ts` is set) and never raises.
- A parent feature issue is never marked code-complete while it still has open sub-issues (#38): `record_code_complete` checks `github_issues.open_sub_issue_count` and reports the remainder as `blocked_by_open_sub_issues`.
- A successful `_MEMORY_MUTATION_METHODS` RPC invalidates that app's `gh_cache` entries (`invalidate_app`), keyed off the coordination repo URL; a new GitHub-mutating memory method the loop can call must be added to that set.
...
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; a route's own 404/validation runs before `cached()`, and any write that changes covered data calls `gh_cache.invalidate_app(app)` (or `invalidate(*keys)`).
...
- Blocking-failure escalations (auth failure, unfixable failure) go through `MemoryIssuesMixin._escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`); don't post a bare Slack message.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) it is served instead, so a dead GitHub token looks like frozen dashboard data, not a 5xx. Durable-layer `put`/`get` swallow all exceptions, and the `gh-cache` DynamoDB table must exist with `expires_at` as its TTL attribute.
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker, not the first; a human answer posted before the latest marker never counts (agentra#54: one old answer had been re-consumed by every later escalation).
- `human_gate` reads the `HUMAN_INPUT_REQUIRED` token from the run's `summary` as well as `error` (a crashed tool call can end a run `completed` with no `error`); a `need_human` label alone doesn't prove an issue was announced — only a recorded Slack thread does.
- `record_failure` now dedups on `_find_similar_open_bug` for both auth and generic unfixable failures (#47): an identical recurring blocker is recorded but not re-escalated while a similar bug is open.
