...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for dashboard GitHub reads: in-process dict over a durable layer — DynamoDB `gh-cache` table in cloud mode, local JSON otherwise); `human_gate/` (raises the human gate for runs that end with a `HUMAN_INPUT_REQUIRED` note); `utils.py`.
...
- A run that ends with `HUMAN_INPUT_REQUIRED` in its `error` (preferred) or `summary` must never go out silently (#55): `human_gate.maybe_raise(run_key)` runs after every successful `record_run` RPC, and `sweep_recent_runs()` re-checks recent runs on each `/trigger/cron` tick as the backstop. It is idempotent per run (`human_input.gate_run_key` + `slack_thread_ts`) and never raises.
- A parent issue with open sub-issues is never marked code-complete: `record_code_complete` checks `github_issues.open_sub_issue_count` and, if >0, skips `mark_code_complete` and returns `blocked_by_open_sub_issues` (#38).
- `record_failure` dedups on `_find_similar_open_bug` for every escalation; both auth-failure and non-auth unfixable failures escalate through `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), so they show on the dashboard's "Needs your input" tab (#42/#46/#47).
...
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; a memory method that mutates GitHub state and is exposed over RPC must also be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py` so `invalidate_app` busts that app's cached views.
...
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker; an answer before it never counts. Anchoring to the first marker let one consumed answer satisfy every later escalation, driving an unattended escalate/false-resume bounce (agentra#54).
...
- `gh_cache.cached` is stale-on-error: if the producer raises, the last cached value is served however old; only a key with no prior value propagates the error. Dashboard views can therefore lag GitHub until the TTL lapses or a whitelisted RPC mutation / app write invalidates them — a direct edit on GitHub does not bust the cache.
