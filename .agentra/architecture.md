...
- Loop list ordering reflects real activity only: `last_run_at` is stamped exclusively by `roll_up_loop` (a run actually completed); administrative writes (`set_loop_status`, `set_loop_pipeline`) never touch it, so a retire/reconcile pass can't reorder the dashboard's Loops list.
...
- GitHub issue label membership checks go through `memory.core._label_names(issue)`, never a raw `LABEL in issue["labels"]` — the real REST payload holds `{"name": ...}` objects, not strings.
...
- Real GitHub issue `labels` are `{"name": ...}` objects, not plain strings (only `github_fake`'s labels are flat strings) — a bare `LABEL in issue["labels"]` silently always returns `False` against real GitHub. `human_input_pending`'s escalation-throttle check had exactly this bug: it never detected the `need_human` label was already applied, so it re-posted the same "Auto-escalated" comment on every cycle (confirmed live, issues #38/#15) until fixed to go through `_label_names(issue)`.
- `list_loops` sorts by `_recency()` = `last_run_at`, not `updated_at` — `updated_at` is bumped by any write, including administrative-only ones (`set_loop_status`/`set_loop_pipeline` from a retire/reconcile pass with no actual run), which would otherwise dump every touched loop to the top of the dashboard's Loops list regardless of age. The DynamoDB `by-app-recency` GSI is still ordered by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH=200`) off the index and re-sorts by `_recency` in Python before truncating to `limit`.
