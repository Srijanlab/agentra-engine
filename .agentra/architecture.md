...
- `agentra/memory/` — per-repo product state. `Memory` = 5 mixins (issues, issue_lifecycle, features, settings, specs); GitHub Issues/Projects-backed. `core.py` holds label constants (incl. the `status:awaiting-testing` label and its legacy `status:shipped` alias) + failure-triage regexes (transient / unfixable-by-agentra / login-required) + spec-header helpers + `pipeline_stages()` (the dashboard's single ordered list of pipeline columns) + `_label_names()` (normalizes an issue's labels to a flat name set — see Gotchas).
...
- `list_loops` orders by real activity, not last write: `last_run_at` is stamped only by `roll_up_loop` on an actual finished run (`_recency()` falls back to `created_at` for a loop that's never run). Administrative-only writes (`set_loop_status`, `set_loop_pipeline` — e.g. a bulk retire/reconcile pass) still bump `updated_at` but never `last_run_at`, so they can't reorder the dashboard's loops list.
...
- Any GitHub issue-label membership check goes through `memory.core._label_names(issue)`, never a bare `LABEL in issue["labels"]` — see Gotchas.
...
- `github_issues.get_issue` returns real GitHub's REST label shape (`[{"name": ..., ...}, ...]`), not flat strings — but `github_fake`'s labels are flat strings, so a bare `LABEL in issue["labels"]` check passes against the test double while always evaluating `False` against real GitHub. `issue_lifecycle.human_input_pending` shipped with exactly this bug: the escalation-comment throttle never detected the `need_human` label was already present, so it re-posted the same "Auto-escalated" comment on every cycle (confirmed live, agentra#38/#15) until fixed to go through `_label_names(issue)`.
- The `by-app-recency` DynamoDB GSI is ordered by `updated_at`, not `last_run_at`; `_query_loops_by_app` over-fetches past `limit` (`_RECENCY_OVERFETCH=200`) and re-sorts by `_recency` in Python — an administrative bulk-touch of a whole app's loops (all bumping `updated_at` together) only breaks ordering if it exceeds that 200-item window.
