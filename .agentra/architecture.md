...
- `agentra/memory/` — per-repo product state. `Memory` = 5 mixins (issues, issue_lifecycle, features, settings, specs); GitHub Issues/Projects-backed. `core.py` holds label constants (incl. the `status:awaiting-testing` label and its legacy `status:shipped` alias, and the `_label_names()` helper for normalizing real-GitHub label objects) + failure-triage regexes (transient / unfixable-by-agentra / login-required) + spec-header helpers + `pipeline_stages()` (the dashboard's single ordered list of pipeline columns).
...
- A loop's `last_run_at` is stamped only by `roll_up_loop` (i.e. a run actually finished/reported back); administrative-only status writes (`set_loop_status`, `set_loop_pipeline`) bump `updated_at` but never `last_run_at`. `list_loops` ordering must key off `last_run_at` (or `created_at` if the loop has never run), not `updated_at`.
...
- Loop recency lives in one place too: `registry.loops._recency()` (falls back `last_run_at` -> `created_at`); every loop-listing path (local sort, `_scan_loops`, `_query_loops_by_app`) sorts through it instead of `updated_at` directly.
- Any label membership check against a real GitHub issue payload (`github_issues.get_issue`, whose `labels` are `{"name": ...}` objects) goes through `memory.core._label_names(issue)`, never a bare `LABEL in issue["labels"]` (that only happens to work against `github_fake`'s flat-string labels).
...
- `github_issues.get_issue`'s `labels` are real-GitHub `{"name": ...}` objects, not plain strings — `human_input_pending`'s bare `_NEED_HUMAN_LABEL in issue["labels"]` always returned `False` against real GitHub (only `github_fake`'s flat-string labels matched), so the throttle guard's dedup check kept re-posting the same "Auto-escalated" comment every cycle instead of once (confirmed live on issues #38 and #15). Fixed via `_label_names(issue)`; any new label check on a real-GitHub payload must go through it too.
- `updated_at` on a loop doc is not "activity": administrative-only writes (`set_loop_status`, `set_loop_pipeline` — used by retire/reconcile passes, not a run) bump it on every loop touched, so sorting `list_loops` by `updated_at` used to shove days-old, already-resolved loops to the top of the dashboard alongside genuinely fresh ones. Sort by `_recency()` (`last_run_at`, stamped only by `roll_up_loop`) instead.
- The DynamoDB `by-app-recency` GSI is still keyed on `updated_at`, not `last_run_at` — `_query_loops_by_app` over-fetches `max(limit, _RECENCY_OVERFETCH=200)` items off the index and re-sorts by `_recency()` in Python to get the true top-`limit`; bumping `limit` well past 200 without also raising the overfetch constant would reintroduce the ordering bug.
