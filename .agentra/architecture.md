...
- A feature parent is never marked code-complete while it still has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` and reports the outstanding count as `blocked_by_open_sub_issues` instead of calling `mark_code_complete` (#38).
- A finished run whose `error`/`summary` contains `HUMAN_INPUT_REQUIRED` must never go out silently: `human_gate.maybe_raise` runs after every successful `record_run` RPC and `human_gate.sweep_recent_runs` re-checks recent runs on each `/trigger/cron` tick; idempotent per run via `loop.human_input.gate_run_key` + `slack_thread_ts` (#55).
- `record_failure` dedups on `_find_similar_open_bug` for every failure class, not just auth: a recurring identical blocker is recorded but not re-escalated. Non-duplicate auth and unfixable failures both escalate via `_escalate_blocking_failure` (thread-mapped Slack notify, human-input context, loop -> `waiting_for_human`) (#46/#47).
- `find_unanswered_human_input_comment` anchors on the MOST RECENT `_HUMAN_INPUT_MARKER` comment, so an already-consumed answer can't satisfy later re-escalations (agentra#54).
...
- A new `Memory` method that changes GitHub Issues/Projects state and is exposed over RPC must also be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`, or the app's `gh_cache` entries stay stale after the write.
- Dashboard GitHub-backed read endpoints go through `gh_cache.cached(key, producer, ttl=None)`; after any write that changes what a key covers, call `invalidate` / `invalidate_app`, and add new per-app keys to `invalidate_app`'s list.
...
- A new GitHub-issue backend call used by `Memory` needs a matching `FakeGitHubBackend` method and an entry in `github_fake.install()`'s patch list (e.g. `open_sub_issue_count`).
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior value exists (however old) it is served, and durable-store put/get/delete failures are swallowed, so a GitHub outage looks like frozen dashboard data, not a 5xx. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300], default 45s.
- RPC cache invalidation only fires when `repo_url` resolves to a registered app's coordination repo (`_app_name_for_coordination_repo`); a mutation against any other URL leaves cached views stale until TTL.
- `human_gate` triggers on the literal `HUMAN_INPUT_REQUIRED` anywhere in a terminal run's `error` (preferred) or free-text `summary`, and a run that reaches it with no known issue files a new `human-input-required` bug. An issue carrying `need_human` but with no recorded Slack thread is treated as never announced and gets its Slack post retried.
