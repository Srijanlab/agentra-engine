...
- `list_loops` ordering is deliberately keyed on `last_run_at` (stamped only by `roll_up_loop`, i.e. a run actually finished), not `updated_at`: an administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a retire/reconcile pass) must not reorder the dashboard's Loops list.
...
- GitHub label membership checks go through `memory.core._label_names(issue)`, never a raw `label in issue["labels"]` — `get_issue`'s real payload has `{"name": ...}` label objects, not the flat strings `github_fake` uses.
...
- `github_issues.get_issue`'s `labels` are `{"name": ...}` objects, not plain strings — a bare `label in issue["labels"]` only ever matched against `github_fake`'s flat-string labels and silently always returned `False` against real GitHub. This left `human_input_pending()` permanently `False`, so the escalation-comment throttle re-posted the same "Auto-escalated" comment on every cycle instead of once (confirmed live, agentra#38/#15) — fixed by routing through `_label_names(issue)`.
- The DynamoDB `by-app-recency` GSI still orders by `updated_at`, not the real-activity-only `last_run_at` `list_loops` actually sorts by; `_query_loops_by_app` compensates by overfetching (`_RECENCY_OVERFETCH`=200 rows) off the index and re-sorting in Python, relying on `updated_at >= last_run_at` always holding.
