...
- A loop's "most recently active" ordering is driven by `last_run_at`, stamped only by `roll_up_loop` on an actual completed run — administrative-only writes (`set_loop_status`, `set_loop_pipeline`) bump `updated_at` but must never affect recency ordering, or a bulk retire/reconcile pass shoves every loop it merely touches to the top of the dashboard's Loops list.
...
- Loop recency (`registry/loops.py::list_loops`, `_scan_loops`, `_query_loops_by_app`) always sorts via the shared `_recency()` helper (`last_run_at` or falls back to `created_at`), never a raw `updated_at` sort — the DynamoDB `by-app-recency` GSI is still keyed on `updated_at`, so the GSI query overfetches (`_RECENCY_OVERFETCH` = 200 rows) and re-sorts in Python before truncating to `limit`.
...
- `github_issues.get_issue`'s real GitHub payload has `labels` as a list of `{"name": ..., ...}` objects, not plain strings — only `github_fake` uses flat strings. A bare `label in issue["labels"]` check silently always misses on real GitHub and only passes against the fake; use `memory.core._label_names(issue)` instead. (Previously bit `human_input_pending`: it always returned `False`, so the escalation-comment throttle guard re-posted the same "Auto-escalated" comment on every cycle instead of once — confirmed live on issues #38/#15.)
- Don't sort loops by raw `updated_at` — any administrative write (`set_loop_status`, `set_loop_pipeline`) bumps it without a real run happening, so a bulk retire pass previously dumped every touched loop, days-old and fresh alike, to the top of the dashboard together. Sort via `_recency()` (`last_run_at`/`created_at`) instead.
