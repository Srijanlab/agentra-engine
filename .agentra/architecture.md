...
- Any GitHub label membership check on a real issue payload goes through `memory.core._label_names(issue)`, never a bare `LABEL in issue["labels"]` — the REST payload's labels are objects, not strings.
...
- `github_issues.get_issue`'s `labels` are raw GitHub REST label objects (`{"name": ..., ...}`), not plain strings. `MemoryIssueLifecycleMixin.human_input_pending`'s stale-escalation guard used to check `_NEED_HUMAN_LABEL in issue["labels"]` directly, which only ever matched `github_fake`'s flat-string labels — against real GitHub it silently always returned `False`, so the guard re-posted an identical "Auto-escalated" comment on every cycle instead of once (confirmed live on agentra#38, #15). Fixed by routing through `_label_names(issue)`; any new label check must do the same.
- `registry/loops.py`'s `list_loops`/`_scan_loops`/`_query_loops_by_app` sort by `_recency` (== `last_run_at`, stamped only by `roll_up_loop` on a real completed run, else `created_at`), not `updated_at`. An administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a bulk retire/reconcile pass) also bumps `updated_at` and used to shove every loop it touched — days-old and freshly-run alike — to the top of the dashboard's Loops list together. The DynamoDB `by-app-recency` GSI still orders by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH = 200`) off the index and re-sorts by `_recency` in Python.
