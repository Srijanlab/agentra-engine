...
- A loop's `updated_at` is bumped by any write, including administrative-only ones (`set_loop_status`, `set_loop_pipeline`) that aren't a real run; only `roll_up_loop` (a run actually completed) stamps `last_run_at`. Loop-list recency (`list_loops`) must key off `last_run_at`, not `updated_at`, or a bulk status/retire pass reorders the whole list.
...
- Checking a real GitHub issue's labels: use `memory.core._label_names(issue)` to get plain strings, never `label in issue["labels"]` directly — `github_issues.get_issue`'s labels are `{name:...}` objects (only `github_fake`'s labels are flat strings).
...
- The DynamoDB `by-app-recency` GSI on the loops table is ordered by `updated_at`, not `last_run_at`; `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH` = 200 items off the index) and re-sorts in Python by `_recency()` to get true run-recency order — raising `limit` much past 200 without raising the overfetch constant would silently mis-order results.
- A real GitHub issue's `labels` are `{name:...}` dict objects from the REST API, not plain strings; a bare `"need_human" in issue["labels"]` silently never matches (only matched `github_fake`'s flat-string labels in tests), so `human_input_pending` always returned `False` against real GitHub and the escalation-comment throttle re-posted "Auto-escalated" every cycle instead of once (agentra#38, #15) — fixed via `_label_names()`, but any new label check must use it too.
