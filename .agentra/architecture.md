...
- `github_issues.get_issue` returns the raw GitHub REST payload, whose `labels` are `{"name": ..., ...}` objects, not plain strings — a bare `LABEL in issue["labels"]` never matches real GitHub (only `github_fake`'s flat-string labels, which is why this previously went uncaught in tests) and silently returns wrong results. `memory.issue_lifecycle` reads labels via `_label_names(issue)`, not raw membership; follow that pattern for any new label check against `get_issue` output.
- `registry.loops.list_loops` sorts by `last_run_at`, stamped only by `roll_up_loop` on a real run — not `updated_at`, which administrative-only writes (`set_loop_status`, `set_loop_pipeline`) also bump. Sorting by `updated_at` previously let a retire/reconcile pass that touched many loops (no real run) shove them all to the top of the dashboard's Loops list together. The DynamoDB `by-app-recency` GSI is still ordered by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH` = 200 rows) past `limit` and re-sorts by `_recency` in Python — don't trust the GSI's own order as final.
