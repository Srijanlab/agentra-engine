...
- `tests/` — pytest suite; `conftest.py` strips prod-pointing env vars at import and autouse-clears the `gh_cache` in-process layer between tests.
...
- A successful memory RPC in `_MEMORY_MUTATION_METHODS` invalidates that app's `gh_cache` entries (`invalidate_app`, resolved from the coordination repo URL), so the dashboard read endpoints don't serve a view the loop just changed.
...
- A parent feature issue is never marked code-complete while it has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` (GraphQL `subIssuesSummary`) on both the sub-feature and `resolves_id` paths, and reports the count as `blocked_by_open_sub_issues` in its return dict.
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker comment and only considers non-internal comments after it, so one already-consumed answer can't satisfy every later re-escalation (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for every failure (not just auth) and returns early on a repeat; a first-seen non-auth unfixable failure escalates through `_escalate_blocking_failure` (thread-mapped Slack notify, human-input context, loop `waiting_for_human`), the same path the auth-failure notice uses (#42/#46/#47).
...
- A new `Memory` method that mutates GitHub Issues/Projects state and is exposed over RPC must also be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`, or the dashboard cache goes stale after it runs.
- Dashboard GitHub-backed read endpoints go through `gh_cache.cached(key, producer)`; a route's own explicit error (e.g. unregistered-app 404) is checked before `cached()`. Writes in routes call `invalidate_app(app)` / `invalidate(*keys)`.
- A new function on `connectors/github_issues.py` that memory code calls needs a matching `FakeGitHubBackend` method registered in `github_fake.install`, or tests hit the real API path.
...
- `gh_cache` hides failures: `cached()` returns the last value however stale when the producer raises (only a never-cached key propagates the error), and the DynamoDB `gh-cache` store's get/put/delete swallow every exception — a missing table or bad creds looks like a cache that never hits. Its in-process layer is a module-level dict, so tests that don't clear it leak entries across cases.
