...
- `list_loops`/`_scan_loops`/`_query_loops_by_app` order by `last_run_at` via `_recency()`, not `updated_at`: `last_run_at` is stamped only by `roll_up_loop` (an actual run finishing), so administrative-only writes (`set_loop_status`, `set_loop_pipeline` — e.g. a retire/reconcile pass) that still bump `updated_at` can't shove a stale, already-resolved loop to the top of the dashboard's Loops list. The DynamoDB `by-app-recency` GSI is keyed on `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH`=200 rows, `updated_at` always >= `last_run_at`) and re-sorts in Python to get true recency order.
...
- Matching against a GitHub issue's labels goes through `memory.core._label_names(issue)`, never a bare membership check on `issue["labels"]` — see Gotchas.
...
- `github_issues.get_issue` returns the raw GitHub REST payload, whose `labels` are `{"name": ..., ...}` objects, not plain strings — a bare `label in issue["labels"]` silently never matches against real GitHub (it only matches `github_fake`'s flat-string labels, which is why this went uncaught in tests for a while). `issue_lifecycle.human_input_pending` had exactly this bug: it always returned `False` against real GitHub, so the `/trigger/cron` throttle guard re-posted the same "Auto-escalated" comment every single cycle instead of once (confirmed live on agentra#38, #15) until fixed to use `_label_names(issue)`.
