...
- `GET /apps/{app}/schedule` is read-only and its `running_best_effort` flag only reflects the current process's `_active_runs` cache — on Vercel serverless that is typically empty, so a running cycle may still report `false`. Its `next_scheduled_run_at` is `null` whenever paused, schedule disabled, or the app has never had a `source="scheduled"` run.
