...
- A multi-part feature's parent is never marked code-complete while it still has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` (both the `sub_feature_of` and `resolves_id` paths) and returns `blocked_by_open_sub_issues` (>0 = skipped) in its result (#38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT `_HUMAN_INPUT_MARKER` comment, so an old already-consumed answer can't satisfy later re-escalations (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for both auth-failure and generic unfixable failures (#47), then escalates via `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), the same wiring as mid-cycle escalations; a repeat of an already-open identical bug is recorded but not re-notified.
- A successful `/internal/rpc` call to a method in `_MEMORY_MUTATION_METHODS` invalidates that app's `gh_cache` entries (`invalidate_app`), so dashboard reads don't serve a view predating the write.
...
- A new `Memory` method the loop calls that changes GitHub Issues/Projects state must go in both `_MEMORY_METHODS` and `_MEMORY_MUTATION_METHODS` (`routes/internal.py`), or the dashboard cache goes stale after it.
- Routes are `APIRouter`s included at root prefix in `server/__init__.py`; auth exemptions go in `auth.py::_PUBLIC_PREFIXES`.
- Dashboard GitHub-backed read endpoints wrap their fetch in `gh_cache.cached(key, producer)`; a producer taking one arg receives the prior etag (return `FetchResult` to store a new one); route-level errors (e.g. unregistered-app 404) are raised before `cached()`. Writes call `invalidate`/`invalidate_app`.
...
- A new function on `connectors/github_issues.py` that memory code calls needs a `FakeGitHubBackend` counterpart registered in `github_fake.install()`, or tests hit the real GraphQL/REST path.
...
- `gh_cache.cached` serves the last known value (however stale) whenever the producer raises, so a persistent GitHub failure shows stale dashboard data with no 5xx; only a key with no prior entry surfaces the error. Cache writes made outside `/internal/rpc` mutation methods or the explicit `invalidate_app` calls in `routes/apps.py` are not seen until the TTL expires.
