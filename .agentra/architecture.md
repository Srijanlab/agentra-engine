...
- Any check of an issue's labels goes through `memory.core._label_names(issue)`, never a bare `LABEL in issue["labels"]` — see Gotchas.
...
- The real GitHub REST API returns `issue["labels"]` as `{"name": ..., ...}` objects, not plain strings (only `github_fake` uses flat strings); a bare `LABEL in issue["labels"]` silently always returns False against real GitHub. `issue_lifecycle.human_input_pending` shipped with exactly this bug, so the escalation-comment throttle never matched and the same "Auto-escalated" comment got re-posted every cycle (agentra#38, #15) — fixed by routing through `_label_names()`.
- `registry/loops.py` `list_loops` sorts by `last_run_at` (stamped only inside `roll_up_loop`, i.e. a real run actually finishing), not `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline` from a retire/reconcile pass) still bumps `updated_at` but must not reorder the "most recently active" list. The DynamoDB `by-app-recency` GSI itself is still keyed on `updated_at`, so `_query_loops_by_app` over-fetches (`_RECENCY_OVERFETCH = 200`) and re-sorts by `_recency` in Python — adding a query path against that GSI without the same re-sort will silently reintroduce the bug.
