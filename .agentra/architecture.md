...
- `agentra/memory/` — per-repo product state. `Memory` = 5 mixins (issues, issue_lifecycle, features, settings, specs); GitHub Issues/Projects-backed. `core.py` holds label constants (incl. the `status:awaiting-testing` label and its legacy `status:shipped` alias) + failure-triage regexes (transient / unfixable-by-agentra / login-required) + spec-header helpers + `pipeline_stages()` (the dashboard's single ordered list of pipeline columns) + `_label_names()` (normalizes a raw GitHub issue's `labels` — dict objects on the real API, plain strings on `github_fake` — into a set of names).
...
- A loop's `last_run_at` is stamped only by `roll_up_loop` when a real run actually finishes — administrative-only writes (`set_loop_status`, `set_loop_pipeline`) bump `updated_at` but never `last_run_at`. `list_loops` (local, `_scan_loops`, and `_query_loops_by_app`) orders by `_recency()` (`last_run_at`, falling back to `created_at`), never bare `updated_at`, so a bulk status/pipeline touch can't reorder the dashboard's "most recently active" list.
...
- Any label-membership check against a raw GitHub issue payload (from `github_issues.get_issue`/`list_issues`) must go through `memory.core._label_names(issue)`, never a bare `LABEL in issue["labels"]` — real GitHub's labels are `{"name": ...}` objects, only `github_fake`'s are flat strings, so the bare form passes tests but silently always returns False against prod.
...
- `github_issues.get_issue`'s raw REST payload has `labels` as `{"name": ...}` objects, not strings — a bare `LABEL in issue["labels"]` matches only `github_fake`'s flat-string labels and always silently returns False against real GitHub. This previously made `human_input_pending`'s throttle guard re-post the same escalation comment every cycle instead of once (fixed via `_label_names`); the trap is generic and applies to any future label check on a real-API issue payload.
- The `loops` table's `by-app-recency` GSI is still keyed on `updated_at`, not `last_run_at` — `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH`=200) off the index and re-sorts by `_recency()` in Python before truncating to `limit`. If an administrative pass touches more than that many loops for one app in a single burst, ordering for the overflow degrades back to GSI (`updated_at`) order.
