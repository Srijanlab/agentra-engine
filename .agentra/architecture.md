...
- GitHub issue labels are always compared via `memory.core._label_names(issue)`, never by indexing `issue["labels"]` directly — real GitHub REST payloads carry label objects (`{"name": ...}`), `github_fake` uses flat strings, and the two shapes are not interchangeable.
...
- The `by-app-recency` DynamoDB GSI is still keyed on `updated_at`, not the real-activity `last_run_at` that `list_loops` actually orders by; `_query_loops_by_app` compensates by overfetching (`_RECENCY_OVERFETCH` = 200 rows) off the GSI and re-sorting in Python, so querying that GSI directly elsewhere would yield the wrong order.
