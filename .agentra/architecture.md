...
- A parent feature issue is never marked code-complete while `github_issues.open_sub_issue_count(...)` > 0: `record_code_complete` checks it on both the sub-feature and `resolves_id` paths and reports the count as `blocked_by_open_sub_issues` in its return dict (#38).
- Dashboard GitHub-backed reads go through `gh_cache.cached`; any write that changes what they cover must call `gh_cache.invalidate_app(app)` (which busts `app_detail:`, `backlog_board:`, `ready_to_review:` for that app plus the shared `digest_batch`). Over RPC this is automatic for names in `_MEMORY_MUTATION_METHODS`.
...
- A new `_MEMORY_METHODS` entry that mutates GitHub Issues/Projects state must also be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`, or the dashboard cache won't be invalidated after the loop calls it.
...
- CI/cron-style callers (`/trigger/cron`, `/trigger/deploy-complete`) are public in `auth.py` and self-gate with `_verify_tick_auth` (`AGENTRA_INTERNAL_TOKEN` or `CRON_SECRET`); `deploy-complete` reuses `_enqueue_cycle(..., enforce_schedule=False)`.
...
- Blocking-failure escalations (auth failure, unfixable failure) go through `Memory._escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), not ad-hoc Slack posts.
...
- `gh_cache.cached` serves the last value however stale if the producer raises (only a never-cached key propagates the error), and the fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300] (default 45s). A GitHub mutation that bypasses `invalidate_app` (e.g. a `Memory` method missing from `_MEMORY_MUTATION_METHODS`, or a change made outside the engine) stays invisible on the dashboard for up to a TTL — or indefinitely while GitHub is failing. Durable-store reads/writes swallow all exceptions.
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker; only non-internal comments after it count. Anchoring to the first marker let one consumed answer satisfy every later escalation, driving an unattended escalate -> false-resume -> re-throttle bounce (agentra#54).
- `Memory.record_failure`'s `_find_similar_open_bug` dedup now gates every escalation (auth and generic unfixable), not just auth: while a similar open bug exists, a recurring identical blocker is recorded but does not re-notify or re-escalate (#47).
