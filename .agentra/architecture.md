...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `utils.py`.
- `agentra/server/gh_cache/` — read-through TTL cache (`cached`, `invalidate`, `invalidate_app`) behind the dashboard's GitHub-backed reads (`apps.py`, `review.py`). Layers: `_inprocess` dict over a durable store (`_dynamo_store` in cloud mode, `_local_store` JSON otherwise); `_ttl` sets the fresh window.
- `agentra/server/human_gate/` — `maybe_raise(run_key)` / `sweep_recent_runs()`: turns a finished run whose `error`/`summary` carries `HUMAN_INPUT_REQUIRED` into a real human gate (need_human label + issue comment, loop `human_input`, Slack thread).
...
- A run that ends terminal with `HUMAN_INPUT_REQUIRED` in its `error` (preferred) or `summary` must never go out silent (agentra#55): `human_gate.maybe_raise` runs right after a successful `record_run` RPC and `sweep_recent_runs` re-checks recent runs on every `/trigger/cron` tick; it is idempotent per `gate_run_key` and never raises.
- A multi-part feature's parent must not be marked code-complete while `open_sub_issue_count > 0`; `record_code_complete` returns `blocked_by_open_sub_issues` instead (#38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker, so an old already-consumed answer can't satisfy later escalations (agentra#54).
...
- Dashboard read endpoints wrap their GitHub fetch in `gh_cache.cached(key, producer)`; any write that changes what a cached key covers must call `gh_cache.invalidate_app(app)` / `invalidate(...)` (the RPC write path and `POST /apps` already do).
...
- Blocking-failure escalation (auth failure, unfixable failure) goes through `MemoryIssuesMixin._escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), not a bare Slack post.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) that value is served, so a GitHub outage shows stale dashboard data rather than a 5xx; only a never-cached key propagates the error. A route's own 404/validation must run before `cached()`.
- `record_failure` now dedups on `_find_similar_open_bug` for every failure class: a repeat of an already-open identical bug records the issue but skips escalation/notification entirely.
- `human_gate` treats a recorded Slack thread (not the `need_human` label) as proof a gate was announced — a label set by an unrelated escalation with no thread still gets its Slack post.
