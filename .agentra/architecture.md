...
<!-- source-sha: c5b7f216ba22f8f55a783348e30bcb4a933655b0 -->
# engine — Architecture
...
- `agentra/server/gh_cache/` — read-through TTL cache (`cached` / `invalidate` / `invalidate_app`) behind the dashboard's GitHub-backed reads (`/apps`, `/apps/{app}`, backlog-board, ready-to-review). Layers: `_inprocess.py` (per-instance dict) over a durable store — `_dynamo_store.py` (`gh-cache` table) in cloud mode, `_local_store.py` (`AGENTRA_HOME/gh-cache/*.json`) otherwise. `_ttl.py` (`AGENTRA_GH_CACHE_TTL_SECONDS`, clamped 10–300s, default 45s), `_entry.py` (`FetchResult` = value + optional ETag).
...
- A successful `/internal/rpc` memory call whose method is in `_MEMORY_MUTATION_METHODS` evicts that app's dashboard cache (`gh_cache.invalidate_app`, app resolved from the coordination `repo_url`); this is how loop-side GitHub writes reach the dashboard's cached reads. `cached()` serves the last known value if its producer raises, so a GitHub outage never 5xxs a cached read once the key has been populated once.
...
- Dashboard reads that hit GitHub go through `gh_cache.cached(key, producer)` with a stable per-app key (`app_detail:{app}`, `backlog_board:{app}`, `ready_to_review:{app}`, shared `digest_batch`) and no per-call `ttl`. Any route or RPC that changes what those views show calls `invalidate_app` (routes use the best-effort `_invalidate_app_cache`); a new `Memory` mutator added to `_MEMORY_METHODS` must also go in `_MEMORY_MUTATION_METHODS`. A one-arg producer receives the previous ETag and may return `FetchResult(value, etag)`.
...
- `gh_cache` now caches off cloud mode too (local JSON under `AGENTRA_HOME/gh-cache/`), so dev/CLI reads can lag up to the TTL, and the in-process dict is a module-level singleton — tests rely on the autouse `_clear_gh_cache_inprocess_layer` fixture, and `AGENTRA_GH_CACHE_TTL_SECONDS` is popped in `conftest.py`.
- Cache eviction only covers writes that pass through `invalidate_app`: routes that call it (`update_app` when `objective` is set, `submit_backlog_request`) and whitelisted memory mutators. Registry-side writes (`record_run`, `set_loop_status`, ...) and any `_MEMORY_METHODS` entry missing from `_MEMORY_MUTATION_METHODS` leave stale entries until TTL. RPC eviction is a silent no-op when `repo_url` doesn't match a registered coordination repo.
- The `digest_batch` key is a single global entry (no longer hashed from the app roster), so every `invalidate_app` busts the whole `/apps` digest, and a newly registered app isn't in it until TTL or the next eviction. Stale-on-error means a persistently failing producer keeps serving old data with only a `logger.warning`.
