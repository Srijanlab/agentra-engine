...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for dashboard GitHub-backed reads: `__init__` exposes `cached` / `invalidate` / `invalidate_app` / `FetchResult`; `_inprocess` dict over a durable store — `_dynamo_store` in cloud mode, `_local_store` JSON under `AGENTRA_HOME/gh-cache` otherwise; `_ttl` reads `AGENTRA_GH_CACHE_TTL_SECONDS`); `utils.py`.
...
- Dashboard read endpoints wrap their producer in `gh_cache.cached(key, producer)` with the default TTL (no per-call `ttl=`); a producer may take one arg (previous etag) and return `FetchResult` for conditional requests. Any write that changes what a cached read covers must call `gh_cache.invalidate_app(app)` best-effort (wrapped so an evict failure never fails the write — see `apps.py::_invalidate_app_cache`); a new per-app cache key must be added to `invalidate_app`'s list, and a new `Memory` mutation the loop calls over RPC must be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`.
...
- `gh_cache.cached()` is stale-on-error: if the producer raises and any prior entry exists (however old), that value is returned with only a log warning; the error propagates only for a never-cached key. The cache is also active off cloud mode now (local JSON under `AGENTRA_HOME/gh-cache`), so dev/CLI/tests see cached reads — tests rely on the autouse conftest fixture clearing `_inprocess`.
- Cache freshness is default 45s (`AGENTRA_GH_CACHE_TTL_SECONDS`, clamped 10–300). `invalidate*` clears the in-process dict only on the instance that handled the write; other warm serverless instances keep their in-process copy until it expires. Writes that bypass the covered paths (direct GitHub edits, RPC methods outside `_MEMORY_MUTATION_METHODS`, `update_app` without an `objective` change) leave reads stale for up to the TTL.
- `/apps` uses the single fixed key `digest_batch`, so `invalidate_app(x)` busts the whole cross-app digest, and the batch is built from the app roster at first-fetch time — a newly registered app isn't in it until the entry expires or is invalidated.
