...
- Loop-list recency (`registry.loops.list_loops`) sorts by `_recency()` = `last_run_at` (stamped only by `roll_up_loop`, i.e. a run actually finished) falling back to `created_at` — never `updated_at`, which administrative-only writes (`set_loop_status`, `set_loop_pipeline`) also bump. The DynamoDB `by-app-recency` GSI is still ordered by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH=200`) and re-sorts by `_recency()` in Python.
...
- `github_issues.get_issue`'s real GitHub REST payload has `labels` as `{"name": ..., ...}` dicts, not plain strings — `LABEL in issue["labels"]` always evaluates `False` against real GitHub (it only matched `github_fake`'s flat-string labels) and silently breaks label-presence checks. Use `memory.core._label_names(issue)` instead; this exact bug made `human_input_pending`'s escalation throttle re-post the same 'Auto-escalated' comment every cycle (agentra#38/#15) until fixed.
