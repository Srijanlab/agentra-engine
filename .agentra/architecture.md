...
- `registry.loops.list_loops` (and its scan/query backends) order by `_recency()` = `last_run_at` (stamped only by `roll_up_loop` when a run actually completes), never bare `updated_at` — administrative-only writes (`set_loop_status`, `set_loop_pipeline`) also touch `updated_at` and must not be able to promote a stale loop to the top of the dashboard's Loops list.
...
- `github_issues.get_issue` returns the raw GitHub REST payload, where `labels` are `{"name": ..., ...}` objects, not plain strings — a bare `LABEL in issue["labels"]` always returns `False` against real GitHub (it only ever matched `github_fake`'s flat-string labels in tests). This is why `human_input_pending`'s old escalation-throttle check silently never matched and re-posted the same "Auto-escalated" comment every cycle (agentra#38/#15); any new label-membership check must go through `memory.core._label_names(issue)`.
- DynamoDB's `loops` table `by-app-recency` GSI is ordered by `updated_at`, not the real-activity `last_run_at` that `list_loops` actually sorts by — `_query_loops_by_app` compensates by overfetching `_RECENCY_OVERFETCH` (200) items off the index and re-sorting in Python, so more than ~200 administrative touches to one app's loops between real runs could still surface a stale loop above a fresh one.
