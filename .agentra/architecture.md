...
- A multi-part feature's parent is never marked code-complete while `github_issues.open_sub_issue_count` > 0 (both the `sub_feature_of` and `resolves_id` paths in `record_code_complete`); the result carries `blocked_by_open_sub_issues` instead (issue #38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT `_HUMAN_INPUT_MARKER` comment, never the first, so one already-consumed answer can't satisfy every later re-escalation (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for both auth-failure and generic unfixable failures (issue #47); a new one escalates through `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), the same wiring as `_escalate_to_human`.
...
- A `_MEMORY_METHODS` entry that changes GitHub Issues/Projects state must also go in `_MEMORY_MUTATION_METHODS` (`routes/internal.py`) so a successful RPC call invalidates that app's `gh_cache` entries.
- Dashboard GitHub-backed read endpoints go through `gh_cache.cached(key, producer)` (a one-arg producer receives the previous etag and may return `FetchResult`); routes that write call `invalidate_app` / `invalidate`. Explicit route errors (e.g. the unregistered-app 404) are raised before `cached()`.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior value exists (however old) it is served, so a GitHub outage shows as stale dashboard data, not a 5xx. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300], default 45s; the durable store's put/get swallow all errors. A write path that bypasses `invalidate_app` (only the `_MEMORY_MUTATION_METHODS` RPC path and `routes/apps.py` do it) stays stale until the TTL lapses, and `invalidate_app` busts the whole shared `digest_batch` key.
- `/trigger/deploy-complete` is in `_PUBLIC_PREFIXES` (skips the Firebase gate) and is protected only by `_verify_tick_auth` inside the handler; it enqueues a deduped cycle with `enforce_schedule=False`, and the payload's `repo`/`sha` are logged, never trusted.
