...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for the dashboard's GitHub-backed reads: `__init__.py` has `cached`/`invalidate`/`invalidate_app`, over `_inprocess` + a durable `_dynamo_store` (`gh-cache` table) / `_local_store` (JSON under `AGENTRA_HOME/gh-cache`), `_ttl.py`, `_entry.py`); `utils.py`.
...
- A feature parent issue is never marked code-complete while it still has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` on both the `sub_feature_of` and `resolves_id` paths, and reports the leftover count as `blocked_by_open_sub_issues` in its return dict (agentra#38).
- `record_failure` dedups on `_find_similar_open_bug` for both auth and generic unfixable failures, so a recurring identical blocker re-notifies at most once; both escalate through `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`) (agentra#42/#46/#47).
...
- A `_MEMORY_METHODS` entry that mutates GitHub Issues/Projects state must also be in `_MEMORY_MUTATION_METHODS` (`routes/internal.py`), so a successful RPC calls `gh_cache.invalidate_app`; a route that writes app state directly does the same (`apps._invalidate_app_cache`, best-effort).
- Dashboard GitHub-backed read endpoints go through `gh_cache.cached(key, producer)`; the key must be one `invalidate_app` clears (`app_detail:`, `backlog_board:`, `ready_to_review:`, `digest_batch`) or be added there.
- Routes are `APIRouter`s included at root prefix in `server/__init__.py`; auth exemptions go in `auth.py::_PUBLIC_PREFIXES`.
- Work-enqueuing endpoints: check `registry.is_paused()` first, dedup with `dedup_key=f"{kind}:{app}"`, record a run via `_new_run_key`, return `{triggered, run_key, job_id, queued}`. `/trigger/deploy-complete` (bearer-gated, CI-called) reuses the same `_enqueue_cycle` with `enforce_schedule=False`.
...
- A new `github_issues` function the memory layer calls needs a matching `FakeGitHubBackend` method and a line in `github_fake.install`'s patch list, or tests hit the real connector.
...
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker, not the first; anchoring to the first let one old answer satisfy every later escalation and drove an unattended escalate -> false-resume -> re-throttle bounce (agentra#54).
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) it is served and the error only logged; only a key with no prior value propagates. TTL is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300], default 45s. Durable-layer put/get failures are swallowed, and the in-process layer is per-instance, so other warm instances keep serving until their own TTL expires after an invalidation.
- RPC cache invalidation only fires when `repo_url` matches an app's coordination repo (`_app_name_for_coordination_repo`); a memory mutation for an unmatched url leaves the dashboard cache stale until TTL.
