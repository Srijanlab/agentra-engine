...
- `registry.loops.list_loops` orders by `last_run_at` (stamped only by `roll_up_loop` when a run genuinely finishes), never by `updated_at` — administrative-only writes (`set_loop_status`, `set_loop_pipeline`, used by retire/reconcile passes) touch `updated_at` but not `last_run_at`, so touching every loop in a bulk pass doesn't reorder the dashboard's Loops list.
...
- GitHub issue label membership is always checked via `memory.core._label_names(issue)` (normalizes the REST payload's `{"name": ...}` label objects to a flat set), never a bare `label in issue["labels"]` — the latter only happens to work against `github_fake`'s flat-string labels, so it silently breaks against real GitHub.
...
- `registry.loops._query_loops_by_app` fetches only `_RECENCY_OVERFETCH` (200) items off the `by-app-recency` GSI (which is ordered by `updated_at`) before re-sorting by `last_run_at` in Python — correct only because `updated_at >= last_run_at` always holds; an app with more than 200 loops touched administratively between real runs could push a genuinely-recent loop outside that window and mis-order the list.
