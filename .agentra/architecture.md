...
- `github_issues.get_issue` returns the raw GitHub REST payload, where `labels` are `{"name": ...}` objects, not plain strings — a bare `LABEL in issue["labels"]` check silently always returns `False` against real GitHub (it only "worked" against `github_fake`'s flat-string labels, which is why a broken `human_input_pending` check went uncaught and re-posted the same "Auto-escalated" comment every cycle instead of once — agentra#38/#15). Use `memory.core._label_names(issue)` when checking labels off a fetched issue.
- `registry.loops.list_loops`/`_scan_loops`/`_query_loops_by_app` sort by `_recency()` (`last_run_at`, stamped only by `roll_up_loop` on a real finished run, falling back to `created_at`), not `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a bulk retire/reconcile pass) bumps `updated_at` on every loop it touches without that reordering the dashboard's Loops list. The `by-app-recency` GSI itself is still ordered by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH` = 200 rows) off the index and re-sorts by `_recency` in Python before truncating to `limit`.
