...
- A loop's `updated_at` is bumped by every write, including administrative-only ones (`set_loop_status`, `set_loop_pipeline`); only `roll_up_loop` (a run actually finishing) stamps `last_run_at`. Anything ordering loops by "recently active" must use `last_run_at`/`_recency()`, not `updated_at`, or a bulk status pass reorders the list incorrectly.
...
- Any code checking whether a GitHub issue carries a given label must go through `memory.core._label_names(issue)`, never `label in issue["labels"]` directly — the real GitHub REST payload's labels are `{name: ...}` objects, and only `github_fake`'s labels are flat strings.
...
- `github_issues.get_issue` returns labels as `{name: ...}` objects; a bare `label in issue["labels"]` check silently always returns False against real GitHub (it only ever matched `github_fake`'s flat-string labels, so this went uncaught in tests). This exact bug in `human_input_pending` made the escalation-comment throttle guard always think no comment had been posted, reposting the same "Auto-escalated" comment every single cycle (agentra#38, #15) until fixed via `_label_names()`.
- The DynamoDB `by-app-recency` loops GSI still indexes on `updated_at`, not the real-activity `last_run_at` that `list_loops` actually sorts by; `_query_loops_by_app` compensates by overfetching `_RECENCY_OVERFETCH` (200) items off the index and re-sorting in Python — bump that constant if a single app ever accumulates more than ~200 loops touched administratively between real runs.
