...
- Loop "most recently active" ordering is driven by `last_run_at`, stamped only by `roll_up_loop` when a real run actually completes — never by `updated_at`, which administrative-only writes (`set_loop_status`, `set_loop_pipeline`, e.g. a bulk retire/reconcile pass) also bump. `registry/loops._recency()` (`last_run_at` else `created_at`) is the single sort key for `list_loops`/`_scan_loops`/`_query_loops_by_app`; don't sort loop lists by `updated_at`.
...
- `github_issues.get_issue` returns the raw GitHub REST payload, whose `labels` are `{"name": ..., ...}` objects, not plain strings. A bare `LABEL in issue["labels"]` check silently always returns `False` against real GitHub (it only ever matched `github_fake`'s flat-string labels, which is why it went uncaught) — this previously made `human_input_pending`'s throttle guard re-post the same "Auto-escalated" comment every cycle instead of once (live on issues #38/#15). Real label-membership checks must go through `memory.core._label_names(issue)`.
- The DynamoDB `by-app-recency` GSI is still keyed on `updated_at`, not the real-activity-only `last_run_at` that `list_loops` actually sorts by — `_query_loops_by_app` compensates by overfetching (`_RECENCY_OVERFETCH=200`) off the index and re-sorting in Python; if the table ever grows past a few hundred loops per app touched administratively at once, this window assumption needs revisiting.
