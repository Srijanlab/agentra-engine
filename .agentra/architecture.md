...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache package for dashboard GitHub reads: in-process dict over a durable store — DynamoDB `gh-cache` table in cloud mode, local JSON otherwise); `human_gate/` (raises the need_human label + Slack post for runs that end asking for a human); `utils.py`.
...
- A terminal run (`completed|failed|blocked|waiting_for_human`) whose `error` (else `summary`) contains `HUMAN_INPUT_REQUIRED` must never go out silently (#55): `human_gate.maybe_raise(run_key)` runs after every successful `record_run` RPC, and `human_gate.sweep_recent_runs()` re-checks recent runs on each `/trigger/cron` tick. It is idempotent per run via `human_input.gate_run_key` + a recorded Slack thread, and never raises.
- A parent feature issue is marked code-complete only when `github_issues.open_sub_issue_count` is 0; otherwise `record_code_complete` skips `mark_code_complete` and returns `blocked_by_open_sub_issues` > 0 (#38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT `_HUMAN_INPUT_MARKER` comment, so an answer consumed by an earlier escalation can't satisfy later ones (agentra#54).
...
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`. A `Memory` method that mutates GitHub issue/project state must be listed in `_MEMORY_MUTATION_METHODS` (`routes/internal.py`) so the RPC path calls `invalidate_app` (busts `app_detail`/`backlog_board`/`ready_to_review`/`digest_batch`); routes that write directly call `invalidate_app` themselves.
- Blocking-failure escalations (`record_failure` auth / unfixable) go through `Memory._escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`); don't post ad-hoc Slack messages. Repeats are suppressed by `_find_similar_open_bug` (#47).
...
- `gh_cache.cached` is stale-on-error: if the producer raises it serves the last cached value however old, and only a never-cached key propagates the error. Fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300] (default 45s); durable-store read/write failures are swallowed, so a write path that skips `invalidate_app` shows stale dashboard data with no error.
- The silent-run token lands in the run's free-text `summary`, not `error` (a crashed tool call ends the run `completed` with no `error`); `human_gate` checks both, preferring `error`. A `need_human` label alone doesn't prove an announcement happened — only a recorded Slack thread does.
