...
- `github_issues.get_issue` returns real GitHub's REST label shape (`{"name": ...}` objects), but `github_fake` returns flat strings — a bare `LABEL in issue["labels"]` matches the fake and silently always returns `False` against real GitHub. Use `memory.core._label_names(issue)` for any label-membership check. Bit us live: `human_input_pending` never matched `need_human`, so `_reconcile_human_input_for_app`'s escalation-comment throttle re-posted the same "Auto-escalated" comment every cron cycle instead of once (issues #38, #15).
- `registry.loops.list_loops` recency is `last_run_at` (stamped only by `roll_up_loop`, i.e. a run actually completed), not `updated_at` — administrative-only writes (`set_loop_status`, `set_loop_pipeline`, e.g. a bulk retire pass) still bump `updated_at` but must not reorder the dashboard's loop list. The DynamoDB `by-app-recency` GSI is still keyed on `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH=200`, safe since `updated_at >= last_run_at` always) and re-sorts by `_recency()` in Python.
