...
- GitHub issue label membership checks always go through `memory.core._label_names(issue)`, never a bare `label in issue["labels"]` — real GitHub returns `{"name": ...}` label objects, not strings (only `github_fake` uses flat strings).
- "Most recently active" ordering on loop docs uses `registry/loops.py::_recency()` (`last_run_at`, stamped only by `roll_up_loop` on a real finished run), never `updated_at`, which is also bumped by administrative-only writes like `set_loop_status`/`set_loop_pipeline`.
...
- GitHub REST label payloads are `{"name": ..., ...}` objects, not plain strings. `issue_lifecycle.py::human_input_pending` used a bare `_NEED_HUMAN_LABEL in issue["labels"]`, which only ever matched `github_fake`'s flat-string labels — against real GitHub it silently always returned `False`, so the escalation-comment throttle re-posted the same "Auto-escalated" comment on every cycle instead of once (confirmed live, agentra#38/#15) until it was fixed to go through `_label_names(issue)`.
- `registry/loops.py::list_loops` orders by `last_run_at`, not `updated_at` — an administrative-only write (`set_loop_status`/`set_loop_pipeline`, e.g. a bulk retire/reconcile pass) bumps `updated_at` on every loop it touches without a real run happening, and used to shove stale loops to the top of the dashboard's Loops list alongside genuinely fresh ones. The DynamoDB `by-app-recency` GSI is still ordered by `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH=200`, since `updated_at >= last_run_at` always) and re-sorts by `_recency()` in Python before truncating to `limit`.
