...
- A loop's "most recently active" timestamp is `last_run_at`, stamped only by `roll_up_loop` when a real run finishes — never by administrative-only writes (`set_loop_status`/`set_loop_pipeline`), which touch `updated_at` instead.
...
- GitHub issue `labels` from the REST API are `{"name": ...}` objects, not strings — always go through `memory.core._label_names(issue)` for membership checks rather than testing a label constant against `issue["labels"]` directly.
- Any new internal/bot-authored comment shape posted to a tracking issue (markers, escalations, etc.) must be added to `github_issue_lifecycle._INTERNAL_COMMENT_PREFIXES` in the same change, or `find_unanswered_human_input_comment` can mistake it for the human's answer.
...
- `get_issue`'s `labels` are `{"name": ...}` dict objects, not plain strings; a bare `LABEL in issue["labels"]` silently always evaluates `False` against real GitHub (it only matched `github_fake`'s flat-string labels, which is why it went uncaught) — confirmed live: the need-human-escalated throttle never fired, posting the same "Auto-escalated" comment 3-4x per cycle (issues #38, #15). Use `memory.core._label_names(issue)`.
- `escalate_existing_issue`'s own diagnosis comment (`"Blocked, needs human input (run X): ..."`) wasn't in `_INTERNAL_COMMENT_PREFIXES`, so once any `Human-Input-Required` marker existed earlier in the thread, `find_unanswered_human_input_comment` read the bot's own later diagnosis comment as the human's answer to it — dispatch immediately resumed the run with its own diagnosis text, hit the same wall, and re-escalated, forever. Confirmed live on agentra#38 (dozens of self-answered bounces over hours); the fix lives in the engine's copy of this file (`_reconcile_human_input_for_app` runs here, not in the loop), so a loop-only fix would leave it live in prod.
- `registry.loops.list_loops` recency: `updated_at` is bumped by administrative-only writes (`set_loop_status`/`set_loop_pipeline`), so a bulk retire/reconcile pass used to shove days-old, already-resolved loops to the top of the list alongside freshly-run ones. Ordering now uses `last_run_at` (stamped only in `roll_up_loop`). The DynamoDB `by-app-recency` GSI is still keyed on `updated_at`, so `_query_loops_by_app` overfetches (`_RECENCY_OVERFETCH = 200`) off the index and re-sorts by real recency in Python — if the table ever gets large this overfetch assumption needs revisiting.
