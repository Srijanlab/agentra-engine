...
- A run that ends (`completed|failed|blocked|waiting_for_human`) with `HUMAN_INPUT_REQUIRED` in its `error` or `summary` must never go out silently (agentra#55): `human_gate.maybe_raise` runs right after a successful `record_run` RPC and again from `sweep_recent_runs()` on each `/trigger/cron` tick. It is idempotent (loop `human_input.gate_run_key` + a recorded Slack thread mean "fully raised"), retries only the missing Slack step, and never raises.
- `record_code_complete` never marks a parent issue code-complete while it still has open sub-issues (`github_issues.open_sub_issue_count`); it returns `blocked_by_open_sub_issues` (>0) instead (#38).
- `record_failure` dedups on `_find_similar_open_bug` for both auth and generic unfixable failures (#47): an already-reported blocker is recorded but not re-escalated. New escalations go through `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), never a bare Slack post.
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker comment, not the first; only non-internal comments after it count as the answer (agentra#54).
...
- Dashboard GitHub-backed reads go through `gh_cache.cached(key, producer)`; any `Memory` method that mutates GitHub state and is exposed over RPC must also be listed in `_MEMORY_MUTATION_METHODS` (`routes/internal.py`), which triggers `invalidate_app`. Direct route writes call `invalidate_app` themselves.
- A new `connectors/github_issues.py` function that `Memory` calls must get a matching `FakeGitHubBackend` method and an entry in `github_fake.install()`'s patch list, or tests hit the real GraphQL/REST path.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior entry exists (however old) that value is returned and the error is only logged, so a persistent GitHub failure shows as frozen dashboard data, not a 5xx. `_dynamo_store` get/put/delete swallow all exceptions, so a missing `gh-cache` table silently degrades to in-process-only caching.
- Cache invalidation over RPC only fires when `repo_url` resolves to an app's coordination repo (`_app_name_for_coordination_repo`); a mutation against any other URL leaves the dashboard cache stale until TTL.
- `human_gate` reads the token from `summary` when `error` is empty — a crashed tool call ends a run as `completed` with no `error`, so checking `error` alone misses it (#49/#50/#51). A `need_human` label alone doesn't prove an announcement happened: only a recorded Slack thread does.
