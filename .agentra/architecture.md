...
- A parent feature issue with open sub-issues is never marked code-complete: `record_code_complete` checks `github_issues.open_sub_issue_count` (GraphQL `subIssuesSummary`) on both the `sub_feature_of` and `resolves_id` paths and reports the count back as `blocked_by_open_sub_issues` (issue #38).
- The dashboard's GitHub-backed reads (`app_detail:*`, `backlog_board:*`, `ready_to_review:*`, shared `digest_batch`) go through `gh_cache.cached`, and every write that changes what they show must evict via `invalidate_app`: the app-config/submit routes in `routes/apps.py`, and any RPC in `_MEMORY_MUTATION_METHODS` (evicts by coordination-repo URL).
...
- A new `Memory` method that mutates GitHub Issues/Projects state and is exposed over RPC goes in both `_MEMORY_METHODS` and `_MEMORY_MUTATION_METHODS` (`routes/internal.py`), or the dashboard cache serves a stale view.
- A new cached read endpoint uses `cached(key, producer)` with a stable `<view>:<app>` key that `invalidate_app` also lists; a producer taking one arg receives the previous etag for conditional requests.
- New GitHub-connector calls need a matching `github_fake.FakeGitHubBackend` method plus an entry in its `install()` patch list (e.g. `open_sub_issue_count`).
...
- `gh_cache.cached` serves the last known value (however stale) whenever the producer raises, so a GitHub outage looks like frozen data, not a 5xx; only a key with no prior entry propagates the error. Its DynamoDB layer's `get`/`put`/per-key delete swallow all errors, so a missing `gh-cache` table silently degrades to in-process-only caching. Any GitHub write that bypasses the routes/RPC mutation paths above stays invisible until the TTL (default 45s) lapses.
- RPC cache eviction only fires when `repo_url` resolves to an app's coordination repo via `_app_name_for_coordination_repo`, and only for methods in `_MEMORY_MUTATION_METHODS`; a mutating method missing from that set doesn't invalidate.
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker, not the first; an issue can be escalated, answered and re-escalated repeatedly, and anchoring to the first let one consumed answer satisfy every later escalation (agentra#54 unattended escalate -> false-resume -> re-throttle bounce).
- `record_failure`'s `already_reported` dedup (`_find_similar_open_bug`) now gates every escalation, not just auth failures: an identical open bug returns early with no Slack notify / human-input context / loop update, so a recurring blocker escalates once. Auth and unfixable failures share `_escalate_blocking_failure`, which needs a non-None issue number and binds/updates the loop as `waiting_for_human` (issues #42/#46/#47).
