...
- Loop recency ordering uses `last_run_at` (stamped only by `registry/loops.py::roll_up_loop` when a run actually completes), never `updated_at` (also bumped by administrative writes like `set_loop_status`/`set_loop_pipeline`) — `list_loops`'s `_recency()` sorts on it, and the DynamoDB `by-app-recency` GSI query (keyed on `updated_at`) overfetches (`_RECENCY_OVERFETCH=200`) and re-sorts in Python to compensate.
- Checking GitHub issue labels from `github_issues.get_issue`'s output must go through `memory.core._label_names(issue)`, not a bare `LABEL in issue["labels"]` — real GitHub returns `{"name": ...}` objects, only `github_fake` uses flat strings.
...
- Checking `_NEED_HUMAN_LABEL in issue["labels"]` directly against real GitHub's payload silently always returns `False` (labels are dict objects, not strings) — this exact bug made the escalation throttle re-post the same "Auto-escalated" comment on every cycle instead of once (agentra#38, #15) before `human_input_pending` was fixed to use `_label_names()`.
- Stamping `last_run_at` from anywhere other than `roll_up_loop` would make an administrative-only status change look like real activity and corrupt `list_loops`' recency ordering — this already happened once when ordering was keyed on plain `updated_at`.
