...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for the dashboard's GitHub-backed reads: `cached` / `invalidate` / `invalidate_app` over an in-process dict + a durable store — DynamoDB `gh-cache` table in cloud mode, local JSON otherwise); `human_gate/` (`maybe_raise` / `sweep_recent_runs` — raises need_human + Slack for a terminal run whose text carries `HUMAN_INPUT_REQUIRED`); `utils.py`.
...
- A terminal run (`completed|failed|blocked|waiting_for_human`) whose `error` or `summary` contains `HUMAN_INPUT_REQUIRED` must never go out silently (#55): `human_gate.maybe_raise` runs right after a successful `record_run` RPC and again via `sweep_recent_runs` on every `/trigger/cron` tick, is idempotent per `gate_run_key`, and treats a recorded Slack thread — not the `need_human` label — as proof an issue was announced.
- A parent issue with open sub-issues is never marked code-complete: `record_code_complete` checks `github_issues.open_sub_issue_count` first and reports `blocked_by_open_sub_issues` instead (#38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker, so an already-consumed answer can't satisfy a later re-escalation (agentra#54).
...
- A `_MEMORY_METHODS` entry that mutates GitHub Issues/Projects state must also go in `routes/internal.py::_MEMORY_MUTATION_METHODS`, so a successful RPC call invalidates that app's `gh_cache` entries (`invalidate_app`); dashboard read endpoints wrap their GitHub fetches in `gh_cache.cached(key, producer)`.
...
- `gh_cache.cached` is stale-on-error: if the producer raises and any prior value exists (however old) it is served, and the fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300] (45s default). A mutation that bypasses `/internal/rpc` or isn't in `_MEMORY_MUTATION_METHODS` leaves the dashboard showing the old view until TTL; the DynamoDB store swallows all errors, so a missing `gh-cache` table degrades silently to always-miss.
- `record_failure` now dedups on `_find_similar_open_bug` for every failure, not just auth: a recurring identical unfixable blocker is escalated (Slack + `waiting_for_human`) once, and later identical failures return early until that bug is cleared (#47).
- `record_code_complete` can return `blocked_by_open_sub_issues > 0` with the parent left un-code-complete — callers must check it rather than assume the feature advanced a stage.
