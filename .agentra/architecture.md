...
- Loop "most recently active" ordering sorts by `last_run_at` (stamped only in `roll_up_loop`, i.e. a real run actually finished), never bare `updated_at` — an administrative-only write (`set_loop_status`, `set_loop_pipeline`) also bumps `updated_at` and must not reorder the dashboard's Loops list; follow `registry/loops.py::_recency` for any new loop-listing code path.
- GitHub issue label payloads from `github_issues.get_issue` are `{"name": ...}` objects, not plain strings — always go through `memory.core._label_names(issue)` rather than checking `label in issue["labels"]` directly.
...
- `github_issues.get_issue`'s raw GitHub REST payload nests labels as `{"name": ..., "color": ..., ...}` objects, not plain strings; a bare `label in issue["labels"]` check only matches `github_fake`'s flat-string labels and silently returns `False` against real GitHub. `human_input_pending`'s escalation-comment throttle had this bug and reposted the same "Auto-escalated" comment on every cycle instead of once (confirmed live on agentra#38/#15) until fixed to use `_label_names(issue)`.
- The `by-app-recency` DynamoDB GSI is keyed on `updated_at`, not the `last_run_at` that `list_loops` actually sorts by — `_query_loops_by_app` compensates by overfetching a window (`_RECENCY_OVERFETCH = 200`) off the index and re-sorting by `_recency` in Python; a single administrative pass touching more than that many loops in one app, or a `limit` above the window, could still surface a stale order.
