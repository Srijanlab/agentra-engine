...
- A parent feature issue is never marked code-complete while it still has open sub-issues: `record_code_complete` checks `github_issues.open_sub_issue_count` (GraphQL `subIssuesSummary`) first and returns `blocked_by_open_sub_issues` (>0 = marking skipped) alongside the issue numbers (#38).
- `find_unanswered_human_input_comment` anchors on the MOST RECENT `_HUMAN_INPUT_MARKER` comment; only non-internal comments after it count as the answer, so an already-consumed answer can't satisfy a later re-escalation (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for both escalation paths; a non-auth unfixable failure escalates through `_escalate_blocking_failure` (thread-mapped Slack notify + human-input context + loop `waiting_for_human`), the same plumbing as the auth-failure notice (#42/#46/#47).
- Every dashboard GitHub-backed read served via `gh_cache.cached` must be busted by any write it covers: RPC memory calls in `_MEMORY_MUTATION_METHODS` call `invalidate_app` for the coordination-repo's app after success; `apps.py` does the same on app writes.
...
- A memory method that changes GitHub Issues/Projects state goes in both `_MEMORY_METHODS` and `_MEMORY_MUTATION_METHODS`; a new cached read key must be added to `gh_cache.invalidate_app`.
- A new `github_issues` function that `Memory` calls needs a `FakeGitHubBackend` counterpart registered in `github_fake.install()`.
...
- `gh_cache.cached` serves the last known value with no age bound when the producer raises (only a key never cached propagates the error). The fresh window is `AGENTRA_GH_CACHE_TTL_SECONDS` (clamped 10–300s, default 45s). `invalidate` clears the in-process dict only on the instance that handled the write, so other warm instances keep serving the old entry until TTL. GitHub edits made outside the engine (a human comment, a manual label change) also only appear after TTL.
- `_dynamo_store` swallows every get/put/delete error, so a missing or misconfigured `gh-cache` table silently degrades the cache to in-process only.
- In `record_code_complete`, `open_sub_issue_count` runs inside the same broad `try` as the writes and after the sub-issue is already closed. A GraphQL failure therefore makes the call log an error and return `None`, and the parent is left unmarked.
