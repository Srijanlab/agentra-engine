...
<!-- source-sha: fb4328d4a8e0a13deda02ea339a3f87a827ba2af -->
# engine — Architecture
...
- `record_failure` files non-transient failures via `record_known_bug` (which dedups on `_find_similar_open_bug`); unfixable-by-agentra ones are filed `needs_human` + `blocking_agentra`. Only login-required failures also send the Slack auth notification, and only when no similar open bug already existed.
...
- `find_unanswered_human_input_comment` flips `marker_seen` on ANY human-input marker and returns the first non-internal, non-blank comment after the first one, never re-anchoring to the latest marker. An old already-consumed answer therefore satisfies every later re-escalation on the same issue (the escalate -> false-resume -> re-throttle bounce of agentra#54).
...
- `record_code_complete` no longer checks for open sub-issues: on the `sub_feature_of` path any call with `more_parts_expected=False` marks the parent code-complete regardless of sibling sub-issues still open (the #38 failure mode where a parent reached `status:awaiting-testing` with unstarted sub-issues).
