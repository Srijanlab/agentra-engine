...
- Dashboard GitHub-backed reads (`/apps`, `/apps/{name}`, backlog board, ready-to-review) go through `gh_cache.cached(key, producer)`: TTL from `AGENTRA_GH_CACHE_TTL_SECONDS` (clamped 10–300s, default 45), and a failing producer serves the last known value (stale-on-error) — only a key with no prior entry propagates the error. Writes that change what they show must call `gh_cache.invalidate_app(app)`; RPC memory calls in `_MEMORY_MUTATION_METHODS` do so automatically by mapping the coordination `repo_url` back to its app.
- A multi-part feature's parent is never marked code-complete while `github_issues.open_sub_issue_count(parent) > 0`; `record_code_complete` returns `blocked_by_open_sub_issues` instead (GitHub issue #38).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker; only non-internal, non-empty comments after it count as an answer, so an old consumed answer can't satisfy later escalations (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug`: a match suppresses all notification/escalation. Otherwise both auth failures and non-auth unfixable failures go through `_escalate_blocking_failure` (thread-mapped Slack notify, human-input context, loop `waiting_for_human`) so replies route and the dashboard shows the block (#42/#46/#47).
...
- A memory method that mutates GitHub Issues/Projects state and is exposed over RPC must be in both `_MEMORY_METHODS` and `_MEMORY_MUTATION_METHODS` (`routes/internal.py`) so the dashboard cache is invalidated; a new per-app `gh_cache` key must be added to `invalidate_app`'s key list; app-write routes in `routes/apps.py` call `_invalidate_app_cache`.
- A new GitHub connector call used by `Memory` needs a matching `FakeGitHubBackend` method and an entry in `github_fake.install`'s patch list (e.g. `open_sub_issue_count`).
...
- `gh_cache` hides GitHub failures: a producer error is only logged (`serving stale value`) and the stale entry is returned indefinitely, and durable-store writes/reads swallow all exceptions. GitHub changes that bypass the engine (direct edits, CLI, memory methods not in `_MEMORY_MUTATION_METHODS`) stay invisible on the dashboard until the TTL lapses.
- `gh_cache.invalidate_app` busts a hard-coded key list (`app_detail`, `backlog_board`, `ready_to_review`, shared `digest_batch`); a new per-app cached key that isn't added there will serve stale data after writes.
- `record_failure`'s dedup now also gates the non-auth unfixable escalation: while a similar open bug issue exists, a recurring blocker is not re-escalated or re-notified, so a mis-matched similarity hit can silently suppress a genuinely new block.
