...
- `loops.list_loops` orders by `_recency()` — `last_run_at` (stamped only by `roll_up_loop`, i.e. a run actually finished) if the loop has ever run, else `created_at` — never plain `updated_at`; an administrative-only field write (`set_loop_status`, `set_loop_pipeline`) also bumps `updated_at` but must not affect recency ordering.
...
- `github_issues.get_issue()` returns the raw GitHub REST payload: `labels` are `{"name": ..., ...}` objects, not plain strings — only `github_fake` uses flat-string labels. A bare `LABEL in issue["labels"]` check silently always returns False against real GitHub; use the `_label_names(issue)` helper. This exact bug in `human_input_pending()` caused the escalation throttle to never trip, re-posting the same "Auto-escalated" comment every cycle (confirmed live on issues #38/#15) until fixed.
- The `by-app-recency` GSI is keyed on `updated_at`, not the real-activity `last_run_at` that `list_loops` actually sorts by — `_query_loops_by_app` compensates by overfetching (`_RECENCY_OVERFETCH = 200`) off the index and re-sorting in Python; if an administrative pass ever touches more than that many loops for one app in a single burst, the true top-`limit` by recency is no longer guaranteed to be inside the fetched window.
