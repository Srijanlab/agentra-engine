...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers, internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache package for the dashboard's GitHub-backed reads: in-process dict over a durable store — `gh-cache` DynamoDB table in cloud mode, JSON files under `AGENTRA_HOME/gh-cache` otherwise); `human_gate/` (raises the need_human label + Slack post for a finished run whose `error`/`summary` carries `HUMAN_INPUT_REQUIRED`); `utils.py`.
...
- `agentra/connectors/` — GitHub App (`github_app.py` mints a per-`owner/repo` installation token), issues/pulls/projects/variables/issue-lifecycle mutations (incl. `migrate_awaiting_testing_label`, the one-time label-rename migration; `github_issues.open_sub_issue_count` via GraphQL `subIssuesSummary`), `github_fake.py`, Slack (`slack.py` + `slack_allowlist.py`).
...
- A run that asks for a human must never end silently (agentra#55): `human_gate.maybe_raise(run_key)` runs after every successful `record_run` RPC and again over recent runs in the `/trigger/cron` tick (`sweep_recent_runs`; result returned as `human_gates`). It qualifies on a terminal status + `HUMAN_INPUT_REQUIRED` in `error` or `summary`, is idempotent per run (`human_input.gate_run_key` + `slack_thread_ts`), and never raises.
- A parent issue with open sub-issues is never marked code-complete (#38): `record_code_complete` checks `github_issues.open_sub_issue_count` on both the `sub_feature_of` and `resolves_id` paths and returns `blocked_by_open_sub_issues` (>0 = not marked).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT `_HUMAN_INPUT_MARKER` comment and only considers comments after it, so an already-consumed answer can't satisfy later re-escalations (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for every failure class: a repeat of an already-open identical bug records but does not re-notify. A non-auth unfixable failure is escalated (#47) through the same `_escalate_blocking_failure` path as the auth failure: thread-mapped Slack notify, human-input context, loop set `waiting_for_human`.
...
- A whitelisted `_MEMORY_METHODS` entry that mutates GitHub issue/project state must also be added to `_MEMORY_MUTATION_METHODS` in `routes/internal.py`, so the RPC handler calls `gh_cache.invalidate_app` for that app.
- Dashboard reads of GitHub-backed data go through `gh_cache.cached(key, producer)` (a one-arg producer receives the prior etag and may return `FetchResult`); routes that write must call `invalidate_app(app)` / `invalidate(...)`.
...
- Any new `github_issues` function that `Memory` calls must also get a matching `FakeGitHubBackend` method and a `github_fake.install()` patch entry, or tests hit the real connector.
...
- `gh_cache.cached` serves the last known value however stale when the producer raises (only a key with no prior entry propagates the error); fresh-window is `AGENTRA_GH_CACHE_TTL_SECONDS` clamped to [10, 300], default 45. Both durable stores swallow every read/write error, so a missing `gh-cache` DynamoDB table degrades silently to per-instance in-process caching. Invalidation deletes the durable entry but cannot clear other warm instances' in-process dicts, which can serve the old value until TTL.
- RPC-side cache invalidation only fires when `repo_url` resolves to a registered app's coordination repo (`_app_name_for_coordination_repo`); an unmatched url or a failed invalidation is logged and skipped, not raised.
- `human_gate` triggers on the `HUMAN_INPUT_REQUIRED` substring anywhere in a run's `error` (preferred) or `summary`; the `need_human` label alone is not treated as "announced" — only a recorded Slack thread for the issue is.
