...
## Module map
- `agentra/server/` — the FastAPI app. `__init__.py` wires middleware + all routers; `auth.py` (Firebase sign-in gate, CORS regex, public-path list); `routes/` (triggers — incl. `/trigger/deploy-complete`, called by a code repo's CI as its last deploy step — internal, apps, schedule, loops, systems, connectors, chat, standup, human_input, review); `state.py` (in-process `_active_runs`/`_app_locks`); `gh_cache/` (read-through TTL cache for dashboard reads: `__init__.py` has `cached`/`invalidate`/`invalidate_app`; `_inprocess`, `_dynamo_store`, `_local_store` layers; `_ttl`, `_entry.FetchResult`); `utils.py`.
...
- Dashboard GitHub-backed reads (`/apps` digest, app detail, backlog board, ready-to-review) go through `gh_cache.cached()`; any write that changes that GitHub state must evict via `invalidate_app`. REST write routes call `_invalidate_app_cache`; RPC memory calls named in `_MEMORY_MUTATION_METHODS` are evicted automatically by coordination `repo_url` after success.
- `record_code_complete` never marks a parent/tracking issue code-complete while `github_issues.open_sub_issue_count` > 0; it returns `blocked_by_open_sub_issues` instead (issue #38: a parent reached `status:awaiting-testing` with sibling sub-issues untouched).
- `find_unanswered_human_input_comment` anchors to the MOST RECENT human-input marker only; an answer to an earlier escalation can't satisfy a later one (agentra#54).
- `record_failure` dedups on `_find_similar_open_bug` for both auth-failure and generic unfixable failures; a repeat blocker records the bug but doesn't re-notify/re-escalate. Both escalation paths go through `_escalate_blocking_failure` (thread-mapped Slack notify, human-input context, loop `waiting_for_human`), so replies route and the dashboard's "Needs your input" tab shows them (issues #42/#46/#47).
...
- A memory method added to `_MEMORY_METHODS` that mutates GitHub Issues/Projects state must also go in `_MEMORY_MUTATION_METHODS`, or the dashboard cache stays stale until TTL.
...
- `gh_cache.cached()` serves the last known value however stale when the producer raises (a GitHub outage shows old data, not a 5xx); only a key never cached lets the error through. Producers taking one arg receive the previous etag. RPC invalidation is skipped when `repo_url` doesn't match a registered coordination repo.
- `open_sub_issue_count` is a live GraphQL call inside `record_code_complete`'s catch-all try block: a GitHub failure there makes the whole call log and return `None`, not just skip the guard.
- `github_fake` mirrors `open_sub_issue_count` via `sub_issue_numbers` on fake issues; a test fixture with no such key sees 0 open sub-issues, so the guard never trips.
