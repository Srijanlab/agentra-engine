...
`AGENTRA_VERIFY_TOKEN` (unset = disabled) enables a read-only user-facing path for black-box checks: send `X-Agentra-Verify-Token: <token>` (constant-time compared) with no Firebase token. It is honored only for `GET /apps`, `GET /apps/{name}/schedule`, `GET /runs/{run_key}`, `GET /openapi.json`, `GET /docs` and `GET /redoc`; any other route or method, and a wrong or empty value, returns 401. It never authorizes `/trigger/cron` or `/internal/*`. It must NOT be set on Production: when `VERCEL_ENV` or `AGENTRA_ENVIRONMENT` is `production`, any request carrying the header gets 403.

The OpenAPI schema and API docs (`/openapi.json`, `/docs`, `/redoc`) are intentionally auth-gated: unauthenticated callers get 401 `authentication_required`. On pre-prod/non-production deployments only, black-box verifiers can read them with `GET` plus the `X-Agentra-Verify-Token` header; production refuses the token with 403, and non-GET methods are never authorized.
