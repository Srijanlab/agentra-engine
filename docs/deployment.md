...
Credentials the Testing Agent needs on pre-prod: `X-Agentra-Verify-Token` (the beta `AGENTRA_VERIFY_TOKEN`) for the read-only GETs, including `GET /loops/{id}/context`, and the pre-prod `AGENTRA_INTERNAL_TOKEN` as an `Authorization: Bearer` for `/internal/loops/{id}/context` and `/internal/rpc`, held by agentra-loop. `GET /health` reports both up front without revealing them: `verify_token_status` (`enabled|not_configured|disabled_in_production`) and a boolean `internal_token_configured` (false means `/internal/*` returns 503 "internal API not configured"). A 401/503 caused by an unprovisioned credential is classified **unverified: credential missing**, not a product failure. Checks that need a Firebase/dashboard credential (e.g. that a context PUT leaves loop status and recency unchanged via `GET /loops/{id}`) are likewise recorded as unverified when only the verify token is held.
