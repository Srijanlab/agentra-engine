...
def feature_branch_name(env: EnvironmentConfig, run_id: str, feature: str) -> str:
    """Dedicated branch for one cycle's feature -- shared by both orchestrator.py's fixed sequence and brain.py's dynamic tool loop."""
...
class SelfHostedVMConfig:
    """The 'self_hosted_vm' deploy_strategy's own connection/topology details for THIS repo's VM -- deliberately not GitHub Variables like EnvironmentConfig's fields: these are infrastructure identifiers tied to a specific, already-provisioned VM, versioned with the code that assumes them, not admin-togglable runtime config."""
...
def load_self_hosted_vm_config(repo: Path) -> SelfHostedVMConfig | None:
    """Reads <repo>/.agentra/memory/architecture/deployment.md's fenced YAML config block -- same fenced-block-in-a-committed-file convention as github_issue_lifecycle.py's record_spec/get_spec (there, GitHub-comment JSON; here, a git-committed YAML block, since these values belong to the repo's own history, not a mutable issue thread)."""
