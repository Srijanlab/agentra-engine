...
| `AGENTRA_VERIFY_TOKEN` | pre-prod ONLY: read-only `X-Agentra-Verify-Token` for `GET /apps`, `/apps/{name}/schedule`, `/runs/{key}` and and the intentionally auth-gated `/openapi.json` schema only (not `/docs` or `/redoc`); GET only, non-production only; never set on Production (refused there with 403); unset = disabled |
