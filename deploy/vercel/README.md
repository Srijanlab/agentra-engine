...
| `AGENTRA_VERIFY_TOKEN` | pre-prod ONLY: read-only `X-Agentra-Verify-Token` for `GET /apps`, `/apps/{name}/schedule`, `/runs/{key}` and the intentionally auth-gated schema/docs `/openapi.json`, `/docs`, `/redoc` (GET only, non-production only); never set on Production (refused there with 403); unset = disabled |
